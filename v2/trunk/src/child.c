...
APTR ImagesWithText(char * ImageNames[], BOOL new)
{
  ...
  result = WindowObject,
	...
	MUIA_Scrollgroup_Contents, VirtGroup = VirtgroupObject,
	  ...
	  Child, bt_slider    = ImageObject,
	    ...
	    ButtonFrame,
	  End,
	  Child, LLabel(GetMUIBuilderString(MSG_Slider)),
  ...
  for(i=0;i<((2*config.columns)-(2*nb_images)%(2*config.columns));i++)
    {
      DoMethod(VirtGroup, OM_ADDMEMBER, HVSpace);
    }

  DoMethod( result, MUIM_Window_SetCycleChain, GR_Scroll,
		  bt_group, bt_keybutton, bt_listview, bt_dirlist, bt_string, bt_label,
		  bt_cycle, bt_radio, bt_image, bt_space, bt_check, bt_slider, bt_gauge, bt_scale,
		  bt_text, bt_prop, bt_rect, bt_colorfield, bt_popasl, bt_popobj, bt_cancel, NULL);
  DoMethod( bt_dirlist, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_DIRLIST);
  DoMethod( bt_text, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_TEXT);
  DoMethod( bt_slider, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_SLIDER);
  DoMethod( bt_image, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_IMAGE);
  DoMethod( bt_scale, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_SCALE);
  DoMethod( bt_keybutton, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_KEYBUTTON);
  DoMethod( bt_listview, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_LISTVIEW);
  DoMethod( bt_check, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_CHECK);
  DoMethod( bt_string, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_STRING);
  DoMethod( bt_group, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_GROUP);
  DoMethod( bt_gauge, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_GAUGE);
  DoMethod( bt_prop, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_PROP);
  DoMethod( bt_cycle, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_CYCLE);
  DoMethod( bt_radio, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_RADIO);
  DoMethod( bt_label, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_LABEL);
  DoMethod( bt_space, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_SPACE);
  DoMethod( bt_rect, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_RECTANGLE);
  DoMethod( bt_colorfield, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_COLORFIELD);
  DoMethod( bt_popasl, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_POPASL);
  DoMethod( bt_popobj, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_POPOBJECT);
  DoMethod( bt_cancel, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_END);
  if (!new) set( bt_group, MUIA_Disabled, TRUE );
...
APTR NewChild( APTR obj , BOOL new)
{
	...
	WI_current = WI_Objects;
        ...
        while (running)
        {
                ...
                {
		case ID_KEYBUTTON:
			keybutton_aux = AllocVec(sizeof(bouton),MEMF_PUBLIC|MEMF_CLEAR);
		...
		case ID_COLORFIELD:
                        ...
                        if (field_aux == NULL)
			{
				...
				field_aux->Help.generated = TRUE;
                        ...
                        break;
		case ID_RECTANGLE:
                        rectangle_aux = AllocVec(sizeof(rectangle),MEMF_PUBLIC|MEMF_CLEAR);
                        if (rectangle_aux == NULL)
			{
				...
				rectangle_aux->Help.generated = FALSE;
                        ...
                        break;
		case ID_LISTVIEW:
			list_aux = AllocVec(sizeof(listview),MEMF_PUBLIC|MEMF_CLEAR);
			if (list_aux == NULL)
			{
				result = NULL;
				break;
		...
		case ID_CHECK:
			...
			if (config.phantom) set(WI_Objects,MUIA_Window_Open,FALSE);
				...
				if (check_aux->title_exist) strcpy(check_aux->Help.title, check_aux->title);
				else strcpy( check_aux->Help.title, "CheckMark Gadget" );
                                result = check_aux;
                                running = FALSE;
                        }
                        else
		...
		case ID_IMAGE:
			...
			{
				if (image_aux->Area.InputMode) image_aux->generated = TRUE;
				else image_aux->generated = FALSE;
		...
		case ID_STRING:
			...
			if (NewString(string_aux,TRUE))
			{
				...
				if (string_aux->title_exist) strcpy(string_aux->Help.title, string_aux->title );
				else strcpy(string_aux->Help.title, "String Gadget" );
				result = string_aux;
				running = FALSE;
			}
			else
		...
		case ID_GROUP:
			...
			{
				group_aux->generated = (group_aux->registermode == TRUE);
				group_aux->Help.generated = TRUE;
				group_aux->Help.nb_char = 0;
				if (strlen(group_aux->Area.TitleFrame)>0)
					strcpy(group_aux->Help.title, group_aux->Area.TitleFrame);
				else strcpy( group_aux->Help.title, "Group" );
				result = group_aux;
				running = FALSE;
			}
			else
...
BOOL EditChild(object *child_aux)
{
	...
	if (child_aux)
	{
		...
		case TY_GROUP:
			...
			bool_aux = NewGroup((group *) child_aux, group_aux->root, FALSE);
			break;
		case TY_KEYBUTTON:
