...
BOOL IsApplicationMenu(menu *menu_aux)
{
  ...
  if (menu_aux == application.appmenu) return(TRUE);
  if ((menu_aux->id == TY_MENU   )||
      (menu_aux->id == TY_SUBMENU)||
      (menu_aux->id == TY_MENUITEM))
...
void ObjectNotify( object *obj )
{
	...
	for(i=0;i<obj->notify->nb_elements;i++)
	{
		...
		case TY_NOTHING:
			DoMethod(obj->muiobj,
			MUIM_Notify,
			GetEvent(NEvenements[ id_src ], 2*evt->src_type+0 ),
			GetEvent(NEvenements[ id_src ], 2*evt->src_type+1 ),
			obj_aux->muiobj,
			GetAction(NActions[ id_dest ], 4*evt->dest_type+0 ),
			GetAction(NActions[ id_dest ], 4*evt->dest_type+1 ),
			GetAction(NActions[ id_dest ], 4*evt->dest_type+2 ),
			GetAction(NActions[ id_dest ], 4*evt->dest_type+3 )
			);
			break;
		case TY_WINOBJ:
