...
BOOL NewColorField( colorfield *field_aux, BOOL new )
{
	...
	if (!(GR_GroupArea = CreateGR_AreaGroup(&field_aux->Area, TRUE, FALSE, TRUE, TRUE, TRUE)))
		return(FALSE);
	...
	WI_ColorField = WindowObject,
		...
		WindowContents, VGroup,
			...
			Child, VGroup,
				Child, HGroup,
					Child, ColGroup(2),GroupFrameT(GetMUIBuilderString(MSG_ColorfieldChoices)),
						MUIA_Weight, 0,
				...
				Child, ColGroup(2), GroupFrameT(GetMUIBuilderString(MSG_Size)),
					...
					Child, Label1(GetMUIBuilderString(MSG_Width)),
                                	Child, STR_width = StringObject,
                                        ...
                                        End,
				End,
				Child, ColGroup(2), GroupFrameT(GetMUIBuilderString(MSG_Indentification)),
	...
	WI_current = WI_ColorField;
        ...
        DoMethod(WI_ColorField, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	...
	DoMethod(STR_label, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_ColorField, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_height);
	DoMethod(STR_height, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_ColorField, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_width);
	DoMethod(STR_width, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_ColorField, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_label );
	...
	if ( !field_aux->fixheight ) set( STR_height, MUIA_Disabled, TRUE );
	if ( !field_aux->fixwidth )  set( STR_width, MUIA_Disabled, TRUE );
	...
	DoMethod(app,OM_ADDMEMBER, WI_ColorField);
        ...
        while (running)
        {
                        ...
                        if ((strlen(field_aux->label)>0))
			{
				...
				get( colorinit, MUIA_Coloradjust_Green, &field_aux->green );
				get( colorinit, MUIA_Coloradjust_Blue, &field_aux->blue );
