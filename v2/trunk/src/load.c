...
{
	if (strcmp( text, "BUILDER_SAVE_FILE") == 0) return(100);
	if (strcmp( text, "BUILDER_SAVE_FILE1.01") == 0) return(101);
	if (strcmp( text, "BUILDER_SAVE_FILE1.02") == 0) return(102);
	if (strcmp( text, "BUILDER_SAVE_FILE1.03") == 0) return(103);
	if (strcmp( text, "BUILDER_SAVE_FILE1.04") == 0) return(104);
	if (strcmp( text, "BUILDER_SAVE_FILE1.10") == 0) return(110);
	if (strcmp( text, "BUILDER_SAVE_FILE1.11") == 0) return(111);
	if (strcmp( text, "BUILDER_SAVE_FILE1.12") == 0) return(112);
	if (strcmp( text, "BUILDER_SAVE_FILE1.13") == 0) return(113);
	if (strcmp( text, "BUILDER_SAVE_FILE1.14") == 0) return(114);
	if (strcmp( text, "BUILDER_SAVE_FILE1.15") == 0) return(115);
	if (strcmp( text, "BUILDER_SAVE_FILE1.16") == 0) return(116);
	if (strcmp( text, "BUILDER_SAVE_FILE1.17") == 0) return(117);
	if (strcmp( text, "BUILDER_SAVE_FILE1.18") == 0) return(118);
	if (strcmp( text, "BUILDER_SAVE_FILE1.19") == 0) return(119);
	if (strcmp( text, "BUILDER_SAVE_FILE1.20") == 0) return(120);
	if (strcmp( text, "BUILDER_SAVE_FILE1.21") == 0) return(121);
	if (strcmp( text, "BUILDER_SAVE_FILE1.22") == 0) return(122);
	if (strcmp( text, "BUILDER_SAVE_FILE1.23") == 0) return(123);
	if (strcmp( text, "BUILDER_SAVE_FILE1.24") == 0) return(124);
	if (strcmp( text, "BUILDER_SAVE_FILE1.25") == 0) return(125);
	if (strcmp( text, "BUILDER_SAVE_FILE1.26") == 0) return(126);
	return(0);
}
...
void ReadHelp( FILE *fichier, APTR obj )
{
	...
	if (version>115) obj_aux->Help.generated = ReadInt(fichier);
	else		 obj_aux->Help.generated = (obj_aux->generated||(obj_aux->Help.nb_char>0));
	if (obj_aux->Help.nb_char>0)
	{
		car = AllocVec(obj_aux->Help.nb_char+1, MEMF_PUBLIC|MEMF_CLEAR);
...
void LoadFile( char *file)
{
        ...
        if ( fichier = fopen( file , "r" ))
	{
		...
		if (version>124) nb_barlabel = ReadInt(fichier);
		if (version>120) nb_label = ReadInt(fichier);
		else nb_label = 0;
		if (version>121)
		{
		...
		else real_getstring[0] = '\0';
		if (version>115)
			strcpy(catprepend, LitChaine(fichier));
		else	strcpy(catprepend, "MSG_");
		i = ReadInt(fichier);
		application.Help.nb_char = i;
		if (version>115) application.Help.generated = ReadInt(fichier);
		if (i>0)
		{
		...
		if (version>104) ReadNotify( fichier, &application );
		if (version>=117)
	        {
	                i = ReadInt(fichier);
	                application.appmenu = LoadObject(fichier, &application, i);
	        }
		...
		else ErrorMessage( GetMUIBuilderString(MSG_NotBuilderSaveFile));
		fclose( fichier );
	}
	}
}
...
void MergeFile( void )
{
        ...
        if ( fichier = fopen( file , "r" ))
	{
		...
		if (version>124) nb_barlabel = nb_barlabel + ReadInt(fichier);
		if (version>120) nb_label = nb_label + ReadInt(fichier);
		if (version>121)
		{
			code          = ReadInt(fichier);
			env           = ReadInt(fichier);
		...
		if (version>115) LitChaine( fichier );
		i = ReadInt(fichier);
		application.Help.nb_char = i;
		if (version>115) ReadInt(fichier);
		if (i>0)
		{
			for(i=0;i<application.Help.nb_char;i++)
			{
...
APTR LoadObject( FILE *fichier, APTR father, int obj )
{
	...
	case TY_GROUP:
		...
		break;
	case TY_KEYBUTTON:
		button_aux = AllocVec(sizeof(bouton),MEMF_PUBLIC|MEMF_CLEAR);
		...
		if (version>104) ReadNotify( fichier, button_aux );
		...
		else	ReadArea( fichier, &button_aux->Area );
		result = button_aux;
		break;
	case TY_GAUGE:
		...
		if (version>104) ReadNotify( fichier, gauge_aux );
		if (version>=113) ReadArea( fichier, &gauge_aux->Area );
		...
		if (version>114) strcpy(gauge_aux->infotext, LitChaine(fichier));
		result = gauge_aux;
		break;
	case TY_IMAGE:
	...
	case TY_CHECK:
		...
		if (version>104) ReadNotify( fichier, check_aux );
		if (version>=113) ReadArea( fichier, &check_aux->Area );
		strcpy(check_aux->title, LitChaine( fichier ));
		check_aux->title_exist = ReadInt( fichier );
		if (version>117) check_aux->init_state = ReadInt( fichier );
		result = check_aux;
		break;
	case TY_SCALE:
		...
		if (version<=114) sprintf(scale_aux->label, "Scale_%d", nb_scale++);
		scale_aux->generated = ReadInt(fichier);
		ReadHelp( fichier, scale_aux );
		if (version>104) ReadNotify( fichier, scale_aux );
		if (version>=113) ReadArea( fichier, &scale_aux->Area );
	...
	case TY_PROP:
		...
		if (version>104) ReadNotify( fichier, prop_aux );
		if (version>=113) ReadArea( fichier, &prop_aux->Area );
		fscanf( fichier, "%d", &i);
	...
	case TY_TEXT:
		...
		break;
	case TY_SLIDER:
		slider_aux = AllocVec(sizeof(slider),MEMF_PUBLIC|MEMF_CLEAR);
		...
		if (version>104) ReadNotify( fichier, slider_aux );
		if (version>=113) ReadArea( fichier, &slider_aux->Area );
	...
	case TY_RADIO:
		...
		break;
	case TY_STRING:
		string_aux = AllocVec(sizeof(chaine),MEMF_PUBLIC|MEMF_CLEAR);
		...
		if (version>104) ReadNotify( fichier, string_aux );
		if (version>=113) ReadArea( fichier, &string_aux->Area );
	...
	case TY_LABEL:
		...
		if (version>104) ReadNotify( fichier, label_aux );
		if (version>=113) ReadArea( fichier, &label_aux->Area );
	...
	case TY_SPACE:
		...
		if (version<=114) sprintf( space_aux->label, "Space_%d", nb_space++);
		space_aux->generated = ReadInt(fichier);
		ReadHelp( fichier, space_aux );
		if (version>104) ReadNotify( fichier, space_aux );
		if (version>118)
		  {
		...
		break;
	case TY_RECTANGLE:
                rect_aux = AllocVec(sizeof(rectangle),MEMF_PUBLIC|MEMF_CLEAR);
		...
		if (version>=113) ReadArea( fichier, &rect_aux->Area);
		fscanf( fichier, "%d", &i );
		if (version<113) i++;
		rect_aux->type = i;
		NextLine( fichier );
		...
		if (version<113)
		{
			...
			NextLine( fichier );
                ...
                break;
	case TY_COLORFIELD:
                field_aux = AllocVec(sizeof(colorfield),MEMF_PUBLIC|MEMF_CLEAR);
		...
		if (version>=113) ReadArea( fichier, &field_aux->Area);
		if (version<113)
		{
	...
	case TY_POPASL:
		...
		break;
	case TY_POPOBJECT:
		popobj_aux = AllocVec(sizeof(popobject),MEMF_PUBLIC|MEMF_CLEAR);
		...
		if (version>=113) ReadArea( fichier, &popobj_aux->Area);
