...
void DeleteApplication(void)
{
	...
	if (application.Help.nb_char>0) FreeVec(application.Help.content);
...
void NewAppli( void )
{
	...
	DoMethod( WI_app, MUIM_Window_SetCycleChain, STR_base, STR_author, STR_title, STR_version, STR_copyright, STR_description, bt_notify, bt_menu, bt_ok, bt_cancel, NULL);
	...
	DoMethod(STR_base, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_app, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_author);
	DoMethod(STR_author, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_app, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_title);
	DoMethod(STR_title, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_app, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_version);
	DoMethod(STR_version, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_app, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_copyright);
	DoMethod(STR_copyright, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_app, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_description);
	DoMethod(STR_description, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_app, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_base);
	DoMethod(WI_app, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_END);
	...
	set(WI_app , MUIA_Window_Open , TRUE);
		...
		case ID_NOTIFY:
			...
			break;
		case ID_NEW_SUBMENU:
			if (config.phantom) set(WI_app, MUIA_Window_Open, FALSE);
			NewMenu(application.appmenu,FALSE);
			WI_current = WI_app;
			if (config.phantom) set(WI_app, MUIA_Window_Open, TRUE);
			break;
		case ID_END:
			running = FALSE;
                	break;
...
void Config( void )
{
	...
	WI_config = WindowObject,
	  ...
	  MUIA_HelpNode, "Prefs",
	  WindowContents, VGroup, Child, RegisterGroup(registertitle),
	    Child, VGroup,
	      Child, ColGroup(4),GroupFrameT(GetMUIBuilderString(MSG_ConfigChoices)),
		Child, HVSpace,
	...
	DoMethod(WI_config, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	DoMethod( IM_geteditor, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_EDIT);
	DoMethod(plist,MUIM_Notify,MUIA_Listview_DoubleClick,TRUE, popup,2,MUIM_Popstring_Close,TRUE);

	DoMethod( WI_config, MUIM_Window_SetCycleChain, CH_phantom, CH_icons, CH_request, CH_test,  SL_depth, popup, STR_editor, IM_geteditor, STR_getstring, bt_ok, bt_save, bt_cancel, NULL );
	...
	set(WI_config , MUIA_Window_Open , TRUE);
		...
		case ID_SAVE:
			...
			Execute( "copy ENV:MUIBuilder.env ENVARC:MUIBuilder.env", 0, 0);
			running = FALSE;
			break;
		case ID_EDIT:
...
LONG AppMsgFunc(struct Hook *hook, APTR obj, struct AppMessage **x)
{
	...
	{
		if ((windows->nb_elements>0)&&(config.request)) bool_aux = RequestMessage(GetMUIBuilderString(MSG_ApplicationLost));
        	if( bool_aux )
        	{
        		Load(obj, buf, FALSE);
        	}
...
void Load( APTR lv_window, char *file, BOOL request )
{
       ...
       for (i=0;i<windows->nb_elements;i++)
       {
      		...
      		DoMethod ( lv_window, MUIM_List_Insert, &aux, 1, MUIV_List_Insert_Bottom);
       }
...
int main( int argc, char *argv[] )
{
	...
	static const struct Hook AppMsgHook = { { NULL,NULL }, (HOOKFUNC)HookEntry, (VOID *)AppMsgFunc, NULL };
	...
	if ( fichier = fopen( "ENV:MUIBuilder.env", "r" ) )
	{
		...
		if (config.labels) config.labels = TRUE;
		fclose( fichier );
	}
	else
	{
	...
	if (argc)
	{
		if (argc>1)
		{
			if (argc>2) ErrorMessage(GetMUIBuilderString(MSG_Arguments));
			{
	...
	DoMethod( bt_appli, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_APPLI);
	DoMethod( bt_new, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_NEWWIN);
	DoMethod( bt_view, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_TEST);
	DoMethod( bt_delete,MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_DELETE);
	DoMethod( bt_code,MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_GENERATE);
	DoMethod( bt_edit,MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_EDIT);
	DoMethod( bt_guide, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_GUIDE);

        /********************************************/
        /* Notifications associees aux Double-Clics */
        /********************************************/

	DoMethod( lv_window, MUIM_Notify, MUIA_Listview_DoubleClick, TRUE, app, 2, MUIM_Application_ReturnID, ID_DBCLICK);
...
#warning unactivated code in the morphos port
	...
	DoMethod( WI_build, MUIM_Window_SetCycleChain, bt_appli, bt_new, bt_edit, bt_delete, bt_view, bt_code, bt_guide, lv_window, NULL);

	DoMethod(lv_window,MUIM_Notify,MUIA_AppMessage,MUIV_EveryTime, lv_window,3,MUIM_CallHook,&AppMsgHook,MUIV_TriggerValue);
	...
	while (running)
	{
		...
		{
		case ID_APPLI:
			if (config.phantom) set(WI_build,MUIA_Window_Open,FALSE);
			NewAppli();
			WI_current = WI_build;
			if (config.phantom) set(WI_build,MUIA_Window_Open,TRUE);
			break;
		case ID_INFO:
		...
		case ID_DBCLICK:
			...
			if ( win_aux != NULL )
			{
				...
				WI_current = WI_build;
				if (config.phantom) set(WI_build,MUIA_Window_Open,TRUE);
				DoMethod( lv_window, MUIM_List_Redraw, MUIV_List_Redraw_Active );
				modify = TRUE;
			}
			break;
		case ID_EDIT:
			...
			else ErrorMessage(GetMUIBuilderString(MSG_SelectWindow));
                        break;
		case ID_TEST:
			if (windows->nb_elements>0)
			{
		...
		case ID_DELETE:
			...
			if (active != MUIV_List_Active_Off)
			{
				...
				DoMethod( lv_window, MUIM_List_Remove, MUIV_List_Remove_Active);
				modify = TRUE;
				}
			}
			else ErrorMessage(GetMUIBuilderString(MSG_SelectWindow));
		...
		case ID_LOAD:
			bool_aux = TRUE;
			if ((windows->nb_elements>0)&&(config.request)&&(modify)) bool_aux = RequestMessage(GetMUIBuilderString(MSG_ApplicationLost));
			if( bool_aux )
			{
			  set(app,MUIA_Application_Sleep, TRUE);
			  set( lv_window, MUIA_List_Quiet, TRUE);
		...
		case ID_GENERATE:
			...
			else ErrorMessage(GetMUIBuilderString(MSG_NeedBase));
			break;
		case ID_NEWAPPLI:
		...
		case ID_END:
			bool_aux = TRUE;
			if (modify) bool_aux = RequestMessage( GetMUIBuilderString(MSG_ConfirmExit));
			if (bool_aux)
			{
				SaveConfig();
				EXIT_PRG();
