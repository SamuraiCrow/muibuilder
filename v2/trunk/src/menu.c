...
LONG MenuDisplayFunc(struct Hook *hook, char **array, menu *menu_aux)
{
	...
	{
		if (!menu_aux->fold) strcpy ( buffer, "\033I[6:30] " );
...
void InitMenuList( APTR list, menu *menu_aux )
{
	...
	{
	case TY_SUBMENU:
		DoMethod(list, MUIM_List_Insert, &menu_aux, 1, MUIV_List_Insert_Bottom);
	case TY_MENU:
		if (menu_aux->fold)
			for(i=0;i<menu_aux->childs->nb_elements;i++)
			{
...
{
	if (menu_aux->father == father) return(TRUE);
	if (menu_aux->id == TY_MENU) return(FALSE);
	return(IsMenuParent(father, menu_aux->father));
}

menu *ParentMenu( menu* menu_aux )
{
	if (menu_aux->id==TY_MENU) return(menu_aux);
	else return(ParentMenu(menu_aux->father));
}
...
void AddToSubMenu( APTR list, menu *father, menu *menu_aux )
{
	...
	for(j=i+1;j<n;j++)
	{
		...
		if (!IsMenuParent(father, tmp_menu)) break;
	}
...
BOOL NewMenu( menu *menu_aux, BOOL new )
{
	...
	obj_aux1 = MUI_MakeObject(MUIO_BarTitle, GetMUIBuilderString(MSG_MenuCreation));
	...
	obj_aux4 = GroupObject,
		...
		Child, ColGroup(2),
	            ...
	            Child, STR_label2 = StringObject, StringFrame, End,
	        End,
	...
	WI_current = WI_Menu;
	DoMethod(WI_Menu, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	...
	DoMethod( BT_NewMenu, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_NEW_MENUTITLE);
	DoMethod( BT_MenuItem, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_NEW_MENUITEM);
	DoMethod( BT_SubMenu, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_NEW_SUBMENU);
	DoMethod( BT_Barlabel, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_NEWBARLABEL);
	DoMethod( LV_Menu, MUIM_Notify, MUIA_Listview_DoubleClick, TRUE, app, 2, MUIM_Application_ReturnID, ID_DBCLICK);
	DoMethod( STR_Entry, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_ACTION);

	DoMethod( CH_Check, MUIM_Notify, MUIA_Selected, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_VALIDATE);
	DoMethod( CH_CheckState, MUIM_Notify, MUIA_Selected, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_VALIDATE);
	DoMethod( CH_Toggle, MUIM_Notify, MUIA_Selected, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_VALIDATE);
	DoMethod( STR_ShortCut, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_VALIDATE);
	DoMethod( STR_label2, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_VALIDATE);
	DoMethod( STR_label2, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_Menu, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_ShortCut);
	DoMethod( STR_ShortCut, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_Menu, 3, MUIM_Set, MUIA_Window_ActiveObject, LV_Menu);

	DoMethod( LV_Menu, MUIM_Notify, MUIA_List_Active, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_UPDATE);
	...
	InitMenuList(LV_Menu, menu_aux);
		...
		{
		case ID_UPDATE:
			DoMethod( LV_Menu, MUIM_List_GetEntry, MUIV_List_GetEntry_Active, &tmp_menu );
			if (tmp_menu)
			{
				if ((tmp_menu->id == TY_SUBMENU)||((tmp_menu->id == TY_MENUITEM)&&(strncmp(tmp_menu->name,"BarLabel",8)==0)))
				{
					set( obj_aux5, MUIA_Disabled, TRUE );
					set( STR_label2, MUIA_String_Contents, tmp_menu->label );
					break;
				}
				else	set( obj_aux5, MUIA_Disabled, FALSE );
				...
				set( CH_CheckState, MUIA_Selected, tmp_menu->check_state );
				set( STR_ShortCut, MUIA_String_Contents, &tmp_menu->key );
				set( STR_label2, MUIA_String_Contents, tmp_menu->label );
			}
			break;
		case ID_VALIDATE:
		...
		case ID_DBCLICK:
			...
			if (active == MUIV_List_Active_Off) break;
			DoMethod( LV_Menu, MUIM_List_GetEntry, MUIV_List_GetEntry_Active, &tmp_menu );
		...
		case ID_ACTION:
			...
			case ID_EDIT:
				...
				{
					DoMethod( LV_Menu, MUIM_List_GetEntry, MUIV_List_GetEntry_Active, &tmp_menu );
					strcpy(tmp_menu->name, (char*)GetStr(STR_Entry));
					DoMethod(LV_Menu, MUIM_List_Redraw, active);
				}
				break;
			case ID_NEW_MENUTITLE:
				if (tmp_menu = AllocVec(sizeof(menu),MEMF_PUBLIC|MEMF_CLEAR))
				{
					InitMenu(tmp_menu);
					...
					StringToLabel(tmp_menu->label, tmp_menu->label);
					DoMethod( LV_Menu, MUIM_List_Insert, &tmp_menu, 1, MUIV_List_Insert_Bottom );
					add(menu_aux->childs, tmp_menu);
					set(LV_Menu, MUIA_List_Active, menu_aux->childs->nb_elements);
					set(WI_Menu, MUIA_Window_ActiveObject, LV_Menu);
				}
				break;
			case ID_NEW_SUBMENU:
				get( LV_Menu, MUIA_List_Active, &active );
				if (active != MUIV_List_Active_Off)
				{
					DoMethod( LV_Menu, MUIM_List_GetEntry, MUIV_List_GetEntry_Active, &father );
				...
				else ErrorMessage(GetMUIBuilderString(MSG_SelectMenuTitle));
				break;
			case ID_NEW_MENUITEM:
				...
				else ErrorMessage(GetMUIBuilderString(MSG_SelectSubMenu));
				break;
			}
		...
		case ID_DELETE:
			...
			if (active != MUIV_List_Active_Off)
			{
				...
				{
					if (tmp_menu == nth(father->childs,i)) break;
				}
		...
		case ID_UP:
			...
			if (active != MUIV_List_Active_Off)
			{
				...
				for(i=0;i<n;i++)
				{
					DoMethod( LV_Menu, MUIM_List_GetEntry, i, &tmp_menu2 );
					if (father == tmp_menu2) break;
				}
		...
		case ID_DOWN:
			...
			if (active != MUIV_List_Active_Off)
			{
				...
				if (i==father->childs->nb_elements-1) break;
				...
				for(j=i+1;j<n;j++)
				{
					DoMethod( LV_Menu, MUIM_List_GetEntry, j, &tmp_menu2 );
					if (tmp_menu2)
					{
