...
void ValidateArea( struct ObjGR_AreaGroup *Obj, area *Area )
{
	...
	if (Area->Background>MUII_SelectedBack) Area->Background = Area->Background + 120;
}

struct ObjGR_AreaGroup * CreateGR_AreaGroup(
		area *Area, BOOL InputMode,
		...
		BOOL Char, BOOL TitleFrame )
{
	...
	if (!(Object = AllocVec(sizeof(struct ObjGR_AreaGroup), MEMF_PUBLIC|MEMF_CLEAR)))
		return(NULL);
	...
	if (Area->Background>MUII_SelectedBack) set(Object->CY_Background, MUIA_Cycle_Active, Area->Background-120);
	else					set(Object->CY_Background, MUIA_Cycle_Active, Area->Background);
