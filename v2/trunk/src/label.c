...
BOOL NewLabel( label *label_aux, BOOL new )
{
	...
	if (!(GR_GroupArea = CreateGR_AreaGroup(&label_aux->Area, FALSE, FALSE, FALSE, TRUE, FALSE)))
		return(FALSE);
	...
	WI_label = WindowObject,
		...
		MUIA_HelpNode, "Label",
			...
			Child, RegGroup = RegisterGroup( RegisterTitles ),
	                        Child, ColGroup(2), GroupFrameT(GetMUIBuilderString(MSG_LabelIdent)),
	...
	DoMethod( bt_ok, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
        ...
        DoMethod(WI_label, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	DoMethod(STR_label, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_label, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_title);
	DoMethod(STR_title, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_label,3, MUIM_Set, MUIA_Window_ActiveObject, STR_label);

	DoMethod( WI_label, MUIM_Window_SetCycleChain, RegGroup, STR_label, STR_title,
