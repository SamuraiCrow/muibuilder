...
BOOL CopyObject( object *objsrc, object *objdest, int taille, APTR father )
{
	...
	{
		helptext = AllocVec(objsrc->Help.nb_char+1, MEMF_PUBLIC|MEMF_CLEAR);
		if (helptext)
		{
...
APTR Duplicate( APTR src, APTR father )
{
	...
	case TY_WINDOW:
		...
		copyok = CopyObject( (object*) win1_aux, (object*) win2_aux, sizeof( window ), father );
		...
		copyok = CopyObject( (object*) &win1_aux->root, (object*) &win2_aux->root, sizeof( group ),father);
		if (!copyok) return(NULL);
		...
		{
			add(win2_aux->root.child, Duplicate(nth(win1_aux->root.child, i), &win2_aux->root) );
		}
	...
	case TY_GROUP:
		...
		copyok = CopyObject( (object*) group1_aux, (object*) group2_aux, sizeof( group ), father );
		if (!copyok) return(NULL);
		...
		{
			add( group2_aux->child, Duplicate( nth( group1_aux->child, i ), group2_aux) );
		}
	...
	case TY_DIRLIST:
		...
		if (!copyok) return(NULL);
                ...
                break;
	case TY_RECTANGLE:
		objdest = AllocVec(sizeof(rectangle),MEMF_PUBLIC|MEMF_CLEAR);
		if (!objdest)
		{
			ErrorMessage(GetMUIBuilderString(MSG_NotEnoughMemory));
			return(NULL);
		}
		copyok = CopyObject( objsrc, objdest, sizeof( rectangle ), father );
		if (!copyok) return(NULL);
		result = objdest;
		break;
	case TY_POPASL:
                objdest = AllocVec(sizeof(popaslobj),MEMF_PUBLIC|MEMF_CLEAR);
		if (!objdest)
		{
			ErrorMessage(GetMUIBuilderString(MSG_NotEnoughMemory));
			return(NULL);
		}
                copyok = CopyObject( objsrc, objdest, sizeof( popaslobj ), father );
		if (!copyok) return(NULL);
                result = objdest;
                break;
	case TY_COLORFIELD:
                objdest = AllocVec(sizeof(colorfield),MEMF_PUBLIC|MEMF_CLEAR);
		if (!objdest)
		{
			ErrorMessage(GetMUIBuilderString(MSG_NotEnoughMemory));
			return(NULL);
		}
                copyok = CopyObject( objsrc, objdest, sizeof( colorfield ), father );
		if (!copyok) return(NULL);
                result = objdest;
                break;
	case TY_POPOBJECT:
		objdest = AllocVec(sizeof(popobject),MEMF_PUBLIC|MEMF_CLEAR);
		if (!objdest)
		{
			ErrorMessage(GetMUIBuilderString(MSG_NotEnoughMemory));
			return(NULL);
		}
                copyok = CopyObject( objsrc, objdest, sizeof( popobject ), father );
		if (!copyok) return(NULL);
	...
	case TY_MENUITEM:
		...
		{
			add(menu2->childs, Duplicate(nth(menu1->childs, i), menu2));
		}
		result = objdest;
		break;
	default:
