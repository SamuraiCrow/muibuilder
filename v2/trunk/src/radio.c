...
queue *CopyStringsList( queue *list )
{
	...
	if (result = create())
		...
		{
			string = nth(list,i);
			if (element = AllocVec(strlen(string)+1,MEMF_PUBLIC|MEMF_CLEAR))
			{
...
APTR LoadRadio( FILE* fichier, APTR father, int version )
{
	...
	if (version>104) ReadNotify( fichier, radio_aux );
	if (version<113)
	{
		strcpy( radio_aux->Area.TitleFrame, LitChaine( fichier ));
		radio_aux->Area.Weight = ReadInt(fichier);
	}
	if (version>=113) ReadArea( fichier, &radio_aux->Area );
	do
	{
...
BOOL NewRadio ( radio *radio_aux, BOOL new)
{
	...
	if (!(GR_GroupArea = CreateGR_AreaGroup(&radio_aux->Area, TRUE, TRUE, TRUE, TRUE, TRUE)))
		return(FALSE);
	...
	RegisterTitles[2] = NULL;
		...
		MUIA_HelpNode, "Radio",
			...
			Child, RegGroup = RegisterGroup(RegisterTitles),
				Child, VGroup,
                        	Child, VGroup,GroupFrameT(GetMUIBuilderString(MSG_RadioEntries)),
					Child, HGroup,
					...
					Child, HGroup,
						...
						Child, STR_entry = StringObject, StringFrame, End,
					End,
				End,
				Child, ColGroup(2),
	...
	DoMethod( bt_new, MUIM_Notify, MUIA_Pressed, FALSE, WI_radio, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_entry);
	...
	DoMethod( lv_entries, MUIM_Notify, MUIA_Listview_DoubleClick, TRUE, WI_radio, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_entry);
        DoMethod(WI_radio, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);

	DoMethod( WI_radio, MUIM_Window_SetCycleChain, RegGroup, lv_entries, bt_new,
		  bt_remove, bt_up, bt_down, STR_entry, STR_label,
        ...
        while (running)
	{
		...
		case ID_DBCLICK:
                        ...
                        set( STR_entry, MUIA_String_Contents, nth( radio_aux->entries, active) );
                        dbclick = TRUE;
                        break;
		case ID_DBCLICK2:
			dbclick = FALSE;
		...
		case ID_DOWN:
                        ...
                        if (active != MUIV_List_Active_Off)
                        {
                                ...
                                if (active < radio_aux->entries->nb_elements) set( lv_entries, MUIA_List_Active, active+1);
                        }
		...
		case ID_DELETE:
                ...
                case ID_OKWIN:
                        ...
                        if ((strlen(aux)>0)&&(radio_aux->entries->nb_elements>0))
			{
				...
				if (new)
                        ...
                        else ErrorMessage(GetMUIBuilderString(MSG_NeedLabelEntries));
                        break;
                case ID_END:
			if (undo)
			{
