...
{
	APTR	WI_PopObject, RegGroup, LV_images, CH_follow, CH_light, CH_Volatile, BT_obj;
	...
	DoMethod( BT_ok, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
        ...
        DoMethod(WI_PopObject, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	DoMethod(IM_obj, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_NEWOBJ );
	DoMethod(BT_obj, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_INFO );
	DoMethod(IM_openhook, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_POPUP1 );
	DoMethod(IM_closehook, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_POPUP2 );
	...
	if (popobj_aux->obj) set( BT_obj, MUIA_Text_Contents, popobj_aux->obj->label );
		...
		case ID_NEWOBJ:
			...
			if (child_aux = NewChild( child_aux, TRUE ) )
			{
				...
				if (child_aux->id != TY_POPOBJECT) nb_popobject--;
			}
			break;
		case ID_POPUP1:
			if (new) GetMUIarg( (object*) popobj_aux, (APTR*) &popobj_aux->openhook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			else GetMUIarg( NULL, (APTR*) &popobj_aux->openhook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			if (popobj_aux->openhook) set(STR_openhook, MUIA_Text_Contents, popobj_aux->openhook);
			else set(STR_openhook, MUIA_Text_Contents, "");
			WI_current = WI_PopObject;
			break;
		case ID_POPUP2:
			if (new) GetMUIarg( (object*) popobj_aux, (APTR*) &popobj_aux->closehook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			else GetMUIarg(NULL, (APTR*) &popobj_aux->closehook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			if (popobj_aux->closehook) set(STR_closehook, MUIA_Text_Contents, popobj_aux->closehook);
			else set(STR_closehook, MUIA_Text_Contents, "");
			WI_current = WI_PopObject;
			break;
		case ID_INFO:
			...
			if (child_aux)
			{
				if (EditChild(child_aux)) set( BT_obj, MUIA_Text_Contents, child_aux->label );
			}
			WI_current = WI_PopObject;
			break;
		case ID_OKWIN:
			...
			else ErrorMessage(GetMUIBuilderString(MSG_GiveObjectAndLabel));
			break;
		case ID_END:
			running = FALSE;
                        break;
