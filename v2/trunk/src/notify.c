...
void LinkNotify( int depart )
{
	...
	for(i=depart;i<labels->nb_elements;i++)
	{
		...
		for(j=0;j<obj_aux->notify->nb_elements;j++)
		{
			...
			{
			case TY_WINOBJ:
				if (!(evt->destination = SearchLabel( evt->label_cible, depart )))
					evt->destination = SearchLabel( evt->label_cible, 0);
			...
			case TY_VARIABLE:
				if (!(evt->destination = SearchLabel( evt->label_cible, depart )))
					evt->destination = SearchLabel( evt->label_cible, 0);
				add( evt->destination->notifysource, obj_aux );
				for(k=0;k<application.Variables->nb_elements;k++)
				{
			...
			case TY_FUNCTION:
				if (!(evt->destination = SearchLabel( evt->label_cible, depart )))
					evt->destination = SearchLabel( evt->label_cible, 0);
				add( evt->destination->notifysource, obj_aux );
				for(k=0;k<application.Functions->nb_elements;k++)
				{
			...
			sprintf(description, "%s (%s) => %s (%s)", Evenements[ obj_aux->id ][ evt->src_type ], obj_aux->label, Actions[ obj_aux2->id ] [ evt->dest_type ], obj_aux2->label );
...
window * ParentWindow( APTR obj )
{
	...
	if (obj_aux->id == TY_WINDOW) return( (window*) obj_aux );
	if (obj_aux->id == TY_APPLI) return(NULL);
...
LONG DisplayFunc(struct Hook *hook, char **array, object *obj)
{
	...
	{
		win_aux = (window*) obj;
		if (!win_aux->deplie) strcpy ( buffer, "\033I[6:30] " );
	...
	{
		buffer[0] = '\0';
		if (obj->id == TY_APPLI) sprintf( buffer2,"\0338%s", obj->label );
		else if (obj->id == TY_GROUP) sprintf( buffer2, "\0338%s", obj->label );
		     else sprintf( buffer2, "%s", obj->label );
	}
...
void InitLabels( APTR obj_aux, window *win, APTR list )
{
	...
	{
	case TY_APPLI:
	  DoMethod( list, MUIM_List_Insert, &obj_aux, 1, MUIV_List_Insert_Bottom );
	  ...
	  for(i=0;i<windows->nb_elements;i++)
	    {
	      ...
	      if (win_aux != win) InitLabels( win_aux, win, list );
	    }
	  break;
	case TY_WINDOW:
	  win_aux = obj_aux;
...
void UpdateLabels( APTR obj, APTR list, ULONG pos )
{
	...
	case TY_WINDOW:
		...
		else
		{
			...
			while((obj_aux->id != TY_WINDOW)&&(obj_aux->id != TY_APPLI))
			{
				DoMethod( list, MUIM_List_Remove, i );
				DoMethod( list, MUIM_List_GetEntry, i, &obj_aux);
				if (!obj) break;
...
void WriteInList( APTR list, char * chaines[] )
{
	...
	{
		DoMethod( list, MUIM_List_Insert, &chaines[i], 1, MUIV_List_Insert_Bottom );
	}
...
{
	if (obj->id<=Current_Max)	WriteInList( list, Evenements[obj->id] );
	else				ErrorMessage(GetMUIBuilderString(MSG_InternalError));
}

void ObjectActions( object * obj, APTR list )
{
	if (obj->id<=Current_Max)	WriteInList( list, Actions[ obj->id ] );
	else                            ErrorMessage(GetMUIBuilderString(MSG_InternalError));
}

BOOL NotifyExist( object *obj, object *obj_dest, int src_type, int dest_type )
{
	int	i;
	BOOL	result = FALSE;
	event	*evenement;
...
BOOL AddArgument( APTR obj, event *evt, int type[] )
{
	...
	case TY_WINOBJ:
		...
		break;
	case TY_FUNCTION:
		result = GetMUIarg( NULL, &evt->arguments, evt->argstring, application.Functions, FALSE, GetMUIBuilderString(MSG_ChoseFunction));
		break;
	case TY_ID:
		result = GetMUIarg( NULL, &evt->arguments, evt->argstring, application.Idents, FALSE, GetMUIBuilderString(MSG_ChoseIdent));
		break;
	case TY_VARIABLE:
		result = GetMUIarg( NULL, &evt->arguments, evt->argstring, application.Variables, FALSE, GetMUIBuilderString(MSG_ChoseVariable));
		break;
	case TY_CONS_INT:
	case TY_CONS_STRING:
	case TY_CONS_CHAR:
...
BOOL EditEvent( event *evt )
{
	...
	{
	case TY_ID:
		result = GetMUIarg( NULL, &evt->arguments, evt->argstring, application.Idents, TRUE, GetMUIBuilderString(MSG_ChoseIdent));
		break;
	case TY_FUNCTION:
		result = GetMUIarg( NULL, &evt->arguments, evt->argstring, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
		break;
	case TY_VARIABLE:
		result = GetMUIarg( NULL, &evt->arguments, evt->argstring, application.Variables, TRUE, GetMUIBuilderString(MSG_ChoseVariable));
		break;
	case TY_CONS_INT:
	case TY_CONS_STRING:
	case TY_CONS_BOOL:
	case TY_CONS_CHAR:
		GetConstant( evt, TYActions[evt->destination->id][ evt->dest_type ], TRUE );
		break;
	}
	return(result);
}
...
BOOL CreateNotify( object * obj, window * win )
{
	...
	DoMethod( BT_quit, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	DoMethod( BT_add, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_NOTIFY);
	DoMethod( BT_remove, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_DELETE);
	DoMethod( LV_evt, MUIM_Notify, MUIA_List_Active, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_EVT);
	DoMethod( LV_cible, MUIM_Notify, MUIA_List_Active, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_NEWOBJ);
	DoMethod( LV_cible, MUIM_Notify, MUIA_Listview_DoubleClick, TRUE, app, 2, MUIM_Application_ReturnID, ID_DEPLIE);
	DoMethod( LV_action, MUIM_Notify, MUIA_List_Active, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_ACTION);
	DoMethod( LV_action, MUIM_Notify, MUIA_Listview_DoubleClick, TRUE, app, 2, MUIM_Application_ReturnID, ID_NOTIFY);
	DoMethod( LV_notify, MUIM_Notify, MUIA_List_Active, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_SEENOTIFY);
	DoMethod( LV_notify, MUIM_Notify, MUIA_Listview_DoubleClick, TRUE, app, 2, MUIM_Application_ReturnID, ID_EDIT);
        DoMethod( WI_notify, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);

	DoMethod( WI_notify, MUIM_Window_SetCycleChain, LV_evt, LV_cible, LV_action, LV_notify, BT_add, BT_remove, BT_quit, NULL );

	/* la liste de tous les labels existants */
	delete_all( labels );
	if (win) CreateLabels( win, NULL );
	for(i=0;i<windows->nb_elements;i++)
        {
	...
	for(i=0;i<obj->notify->nb_elements;i++)
	{
		evenement = nth( obj->notify, i );
		obj_aux = evenement->destination;
		sprintf(description, "%s (%s) => %s (%s)", Evenements[ obj->id ][ evenement->src_type ], obj->label, Actions[ obj_aux->id ] [ evenement->dest_type ], obj_aux->label );
		strncpy( evenement->description, description, 120 );
		chaine_aux = (char*) evenement->description;
		DoMethod( LV_notify, MUIM_List_Insert, &chaine_aux, 1, MUIV_List_Insert_Bottom);
	}
	...
	set( LV_evt, MUIA_List_Active, 0 );
		...
		{
		case ID_DEPLIE:
			DoMethod( LV_cible, MUIM_List_GetEntry, MUIV_List_GetEntry_Active, &obj_aux );
			if (obj_aux)
			{
		...
		case ID_SEENOTIFY:
			...
			{
			  evenement = nth( obj->notify, active );
			  if ((evenement->destination->id!=TY_WINDOW)&&(evenement->destination->id!=TY_APPLI))
			    {
				...
				if (!win_aux->deplie)
				  {
				    set( LV_cible, MUIA_List_Quiet, TRUE );
				    set( LV_action, MUIA_List_Quiet, TRUE );
				    UpdateLabels( win_aux, LV_cible, Position( win_aux, LV_cible ) );
				    set( LV_cible, MUIA_List_Quiet, FALSE );
				    set( LV_action, MUIA_List_Quiet, FALSE );
				  }
			      i = Position( evenement->destination, LV_cible );
			    }
			  else i = Position( evenement->destination, LV_cible );
			  if (i != -1)
			    {
			      ...
			      set( LV_action, MUIA_List_Active, evenement->dest_type );
			    }
			}
			break;
		case ID_EDIT:
			...
			{
				evenement = nth( obj->notify, active );
				if (!EditEvent(evenement)) DeleteNotification(LV_notify, obj, active);
			}
			WI_current = WI_notify;
			break;
		case ID_DELETE:
			...
			if (active != MUIV_List_Active_Off)
			{
			  ...
			  if (bool_aux) DeleteNotification(LV_notify, obj, active);
			}
			else ErrorMessage(GetMUIBuilderString(MSG_SelectNotifyLink));
			break;
		case ID_NOTIFY:
			...
			if (active != MUIV_List_Active_Off)
			{
			  ...
			  if (active2 != MUIV_List_Active_Off)
			    {
			      DoMethod( LV_cible, MUIM_List_GetEntry, MUIV_List_GetEntry_Active, &obj_aux );
			      sprintf(description, "%s (%s) => %s (%s)", Evenements[ obj->id ][ active ], obj->label, Actions[ obj_aux->id ] [ active2 ], obj_aux->label );
			      evenement = AllocVec(sizeof(event),MEMF_PUBLIC|MEMF_CLEAR);
			      ...
			      if (AddArgument( obj_aux, evenement, TYActions[obj_aux->id] ))
				{
				  add( obj->notify, evenement );
				  chaine_aux = (char*) evenement->description;
				  set( LV_notify, MUIA_List_Quiet, TRUE);
				  DoMethod( LV_notify, MUIM_List_Insert, &chaine_aux, 1, MUIV_List_Insert_Bottom);
				  set(LV_notify, MUIA_List_Active, obj->notify->nb_elements);
				  set( LV_notify, MUIA_List_Quiet, FALSE);
				  add( obj_aux->notifysource, obj );
				}
			      else FreeVec( evenement );
			    }
			  else ErrorMessage(GetMUIBuilderString(MSG_SelectAction));
			}
			else ErrorMessage(GetMUIBuilderString(MSG_SelectEvent));
			WI_current = WI_notify;
			break;
		case ID_NEWOBJ:
			DoMethod( LV_cible, MUIM_List_GetEntry, MUIV_List_GetEntry_Active, &obj_aux );
