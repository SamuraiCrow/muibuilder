...
BOOL NewScale( scale *scale_aux, BOOL new )
{
	...
	if (!(GR_GroupArea = CreateGR_AreaGroup(&scale_aux->Area, TRUE, TRUE, TRUE, FALSE, FALSE)))
		return(FALSE);
	...
	WI_scale = WindowObject,
		...
		MUIA_HelpNode, "Scale",
			...
			Child, RegGroup = RegisterGroup(RegisterTitles),
	                        Child, VGroup,
				Child, ColGroup(4),GroupFrameT(GetMUIBuilderString(MSG_Orientation)),
					Child, HVSpace,
	...
	DoMethod(WI_scale, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
