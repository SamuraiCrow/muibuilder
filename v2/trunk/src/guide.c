...
void Modify( APTR obj )
{
	...
	DoMethod(WI_modify, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_END);

	DoMethod( WI_modify, MUIM_Window_SetCycleChain, STR_title, CH_generated, LV_text, BT_edit, BT_ok, BT_cancel , NULL );

	sprintf(buffer, "%s : %d", GetMUIBuilderString(MSG_TextLength), obj_aux->Help.nb_char);
	set( TX_length, MUIA_Text_Contents, buffer );
	if (obj_aux->Help.nb_char>0) set( LV_text, MUIA_Floattext_Text, obj_aux->Help.content);
	...
	while (running)
	{
		...
		case ID_NEWGRP:
			...
			set( TX_length, MUIA_Text_Contents, buffer );
			if (obj_aux->Help.nb_char>0) set( LV_text, MUIA_Floattext_Text, obj_aux->Help.content);
			else set( LV_text, MUIA_Floattext_Text, NULL);
			break;
		case ID_END:
			running = FALSE;
			break;
...
void GuideReference( APTR obj, FILE *fichier)
{
  ...
  switch(obj_aux->id)
    ...
    case TY_GROUP:
      group_aux = obj;
      if ((group_aux->Help.generated)&&(group_aux->Help.nb_char>0))
...
void GenerateGuide( APTR obj )
{
	...
	case TY_GROUP:
		...
		if ((group_aux->Help.generated)&&(group_aux->Help.nb_char))
		  {
		    fprintf( ficguide, "@NODE %s \"%s\"\n", group_aux->label,group_aux->Help.title);
	...
	case TY_POPOBJECT:
		...
		if ((obj_aux->Help.nb_char>0)&&(obj_aux->Help.generated))
		  {
		    fprintf( ficguide, "@NODE %s \"%s\"\n", obj_aux->label,obj_aux->Help.title);
...
void EditNode( APTR obj )
{
	...
	if (obj_aux->Help.nb_char>0)
	{
		...
		else ErrorMessage(GetMUIBuilderString(MSG_CantOpen));
		fclose( fichier );
	}
	set( app, MUIA_Application_Sleep, TRUE );
	...
	SystemTags( buffer,
        	...
        	TAG_DONE );
	set( app, MUIA_Application_Sleep, FALSE );
	if (obj_aux->Help.nb_char>0) FreeVec(obj_aux->Help.content);
	if (alloc = Open( "T:MUIBuilderGuide.tmp", MODE_OLDFILE ))
	{
		ExamineFH( alloc, &Info );
		obj_aux->Help.nb_char = Info.fib_Size;
		obj_aux->Help.content = AllocVec(Info.fib_Size+1, MEMF_PUBLIC|MEMF_CLEAR);
		Read( alloc, obj_aux->Help.content, obj_aux->Help.nb_char);
		aux = (char *) ((ULONG) obj_aux->Help.content + obj_aux->Help.nb_char);
		*aux = '\0';
		Close( alloc );
	}
...
void EditAppNode( void )
{
	...
	Execute( buffer, 0, 0);
        ...
        if (alloc = Open( "T:MUIBuilderGuide.tmp", MODE_OLDFILE ))
        {
                ...
                application.Help.content = AllocVec(Info.fib_Size+1, MEMF_PUBLIC|MEMF_CLEAR);
                Read( alloc, application.Help.content, application.Help.nb_char);
		aux = (char *) ((ULONG) application.Help.content + application.Help.nb_char);
		*aux = '\0';
                Close( alloc );
        }
        DeleteFile( "T:MUIBuilderGuide.tmp" );
...
void AddObject( APTR obj, APTR list )
{
	...
	case TY_GROUP:
		...
		if (!group_aux->root)
		{
			...
			DoMethod( list, MUIM_List_Insert, &chaine_aux, 1, MUIV_List_Insert_Bottom);
		}
		for(i=0;i<group_aux->child->nb_elements;i++)
		{
...
void Guide()
{
	ULONG	signal;
	BOOL	running = TRUE;
	APTR	WI_guide, LV_windows, LV_childs, BT_objnode, BT_appnode, BT_winnode, BT_view;
				...
				Child, VGroup,
					Child, LV_childs = ListviewObject,
						MUIA_Listview_List, ListObject,
							...
							End,
						End,
					Child, bt_edit2 = MUI_MakeObject(MUIO_Button, GetMUIBuilderString(MSG_Edit2)),
					End,
				End,
			Child, GroupObject,
	...
	DoMethod( BT_objnode, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_NEWOBJ );
	DoMethod( BT_appnode, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_NEWGRP );
	DoMethod( BT_winnode, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_NEWWIN );
	DoMethod( BT_gen, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_GENERATE);
	DoMethod( BT_genwin, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_GENERATEWIN);
	DoMethod( BT_genobj, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_FINTEST );
	DoMethod( BT_view, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_INFO );
	DoMethod( LV_windows, MUIM_Notify, MUIA_List_Active, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_GROUP);
	DoMethod( LV_windows, MUIM_Notify, MUIA_Listview_DoubleClick, TRUE, app, 2, MUIM_Application_ReturnID, ID_DBCLICK);
	DoMethod( bt_edit, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_DBCLICK);
	DoMethod( LV_childs, MUIM_Notify, MUIA_Listview_DoubleClick, TRUE, app, 2, MUIM_Application_ReturnID, ID_DBCLICK2);
	DoMethod( bt_edit2, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_DBCLICK2);
	DoMethod(WI_guide, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_END);

	DoMethod( WI_guide, MUIM_Window_SetCycleChain, LV_windows, LV_childs, BT_appnode, BT_winnode, BT_objnode, BT_gen, BT_genwin, BT_genobj, BT_view, BT_quit, bt_edit, bt_edit2, NULL );

	for(i=0;i<windows->nb_elements;i++)
	{
		win_aux = nth( windows, i );
		chaine_aux = win_aux->label;
		DoMethod( LV_windows, MUIM_List_Insert, &chaine_aux, 1, MUIV_List_Insert_Bottom);
	}
	...
	while (running)
	{
		...
		case ID_INFO:
			...
			if ( Requester( GetMUIBuilderString(MSG_AmigaguideDocName), guidenamedir, name ) )
				...
				sprintf( buffer, "SYS:Utilities/Multiview \"%s\"", filename);
				SystemTags( buffer,
                            	SYS_Input, Open("NIL:", MODE_NEWFILE),
                            	SYS_Output, Open("NIL:",MODE_NEWFILE),
                            	SYS_Asynch, TRUE,
		...
		case ID_GENERATE:
			...
			else ErrorMessage(GetMUIBuilderString(MSG_CantOpenAmigaGuide));
			}
			break;
		case ID_FINTEST:
			get( LV_childs, MUIA_List_Active,&active);
