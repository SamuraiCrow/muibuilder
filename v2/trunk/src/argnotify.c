...
APTR PointerOnString(char *chaine, queue *list)
{
	...
	if (i != list->nb_elements) return(result);
	else			    return(NULL);
}
	...
	for(i=0;i<labels->nb_elements;i++)
	{
		...
		if (!result)
			for(j=0;j<obj->notify->nb_elements;j++)
			{
				...
				case TY_ID:
				case TY_VARIABLE:
					if (chaine==evt->arguments) result=obj;
					break;
				}
				if (result) break;
			}
...
BOOL GetMUIarg( object *obj_aux, APTR *stringptr, char *stringname, queue *list, BOOL old, char *title )
{
	...
	DoMethod( WI_MUIarg, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_END);
	DoMethod( LV_labels, MUIM_Notify, MUIA_Listview_DoubleClick, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	DoMethod( LV_labels, MUIM_Notify, MUIA_List_Active, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_INFO);
	DoMethod( STR_add, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_NEWOBJ );
	...
	DoMethod( WI_MUIarg, MUIM_Window_SetCycleChain, LV_labels, STR_add, BT_remove, BT_ok, BT_cancel, NULL);

	delete_all(labels);
	CreateLabels(&application, NULL);
	if (obj_aux) CreateLabels(obj_aux, NULL);
	for(i=0;i<list->nb_elements;i++)
	{
	...
	while (running)
	{
		...
		{
		case ID_INFO:
			DoMethod(LV_labels, MUIM_List_GetEntry, MUIV_List_GetEntry_Active, &chaine_aux);
		...
		case ID_OKWIN:
			...
			{
				if (stringptr) *stringptr = nth( list, active );
				if (stringname) strcpy(stringname, nth( list, active ));
				set(TX_Text, MUIA_Text_Contents, *stringptr);
			}
			else ErrorMessage(GetMUIBuilderString(MSG_SelectIdentifier));
			break;
		case ID_NEWOBJ:
			...
			if (strlen(chaine_aux)>0)
			{
				...
				{
					label_aux = AllocVec(strlen(chaine_aux)+1,MEMF_PUBLIC|MEMF_CLEAR);
					if (!label_aux)
					{
					...
					add(list, label_aux);
					DoMethod( LV_labels, MUIM_List_Insert, &label_aux, 1, MUIV_List_Insert_Bottom);
					set(LV_labels, MUIA_List_Active, MUIV_List_Active_Bottom);
				}
				else ErrorMessage( GetMUIBuilderString(MSG_IdentifierExists));
			}
			break;
		case ID_DELETE:
			get( LV_labels, MUIA_List_Active, &active );
			if (active != MUIV_List_Active_Off)
			{
				...
				if (!FindArgument(nth(list, active)))
				{
					...
					DoMethod( LV_labels, MUIM_List_Remove, MUIV_List_Remove_Active);
					set(LV_labels, MUIA_List_Active, MUIV_List_Active_Off);
				}
			}
			else ErrorMessage(GetMUIBuilderString(MSG_SelectIdentifier));
			break;
...
BOOL GetConstant( event *evt, int Type, BOOL old )
{
	...
	DoMethod(bt_ok, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	DoMethod(bt_cancel, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_END);
	DoMethod(WI_Constant, MUIM_Window_SetCycleChain, STR_Integer, STR_String, STR_Char, CY_Bool, bt_ok, bt_cancel, NULL);

	switch(Type)
	{
	case TY_CONS_INT:
		if (old) set(STR_Integer, MUIA_String_Contents, evt->argstring);
		ActiveObject = STR_Integer;
		break;
	case TY_CONS_BOOL:
		if (old) set(CY_Bool, MUIA_Cycle_Active, atoi(evt->argstring));
		ActiveObject = CY_Bool;
		break;
	case TY_CONS_STRING:
		if (old) set(STR_String, MUIA_String_Contents, evt->argstring);
		ActiveObject = STR_String;
		break;
	case TY_CONS_CHAR:
		if (old) set(STR_Char, MUIA_String_Contents, evt->argstring);
	...
	while (running)
	{
		...
		case ID_OKWIN:
			...
			case TY_CONS_BOOL:
				...
				break;
			default:
				get(ActiveObject, MUIA_String_Contents, &string_aux);
