...
APTR LoadGroup( FILE* fichier, APTR father, int version)
{
	...
	if (version>104) ReadNotify( fichier, group_aux );
	if (version>=113) ReadArea(fichier, &group_aux->Area);
	if (version<113) strcpy(group_aux->Area.TitleFrame , LitChaine( fichier ));
	group_aux->root = ReadInt(fichier);
	if ( !group_aux->root ) add( current_window->groups, group_aux);
	if (version<113)
	{
		title_exist = ReadInt(fichier); /* obsolete title_exist */
		if (title_exist) group_aux->Area.Frame = MUIV_Frame_Group;
		else group_aux->Area.TitleFrame[0] = '\0';
	}
	...
	if (version<113) group_aux->Area.Weight = ReadInt(fichier);
	...
	do
	{
		...
		if ( i!= -1 )
		{
			if ( !group_aux->root ) obj_aux = LoadObject( fichier, group_aux, i );
			else obj_aux = LoadObject( fichier, &current_window->root, i );
	...
	group_aux->horizontal = (group_aux->horizontal&&(!group_aux->rows)&&(!group_aux->columns));
	return(group_aux);
}
...
BOOL NewGroup( group *group_aux, int root , BOOL new )
{
	...
	APTR 	STR_label, STR_rownumber, gr_buttons;
 	APTR    GR_register,lv_entries,bt_new, bt_remove, bt_up, bt_down, STR_entry;
	BOOL 	running = TRUE;
	BOOL	result = FALSE;
	ULONG 	signal, active;
	int	i, n;
	...
	GR_register = GroupObject,
		...
		End,
		Child, ColGroup( 2 ),
			Child, Label2( GetMUIBuilderString(MSG_Entry) ),
		...
		WindowContents, VGroup,
			Child, RegGroup1 = RegisterGroup(RegisterTitles),
				Child, ColGroup(2),
					...
					Child, ColGroup(2),GroupFrameT( GetMUIBuilderString(MSG_GroupAttributes)),
						MUIA_HorizWeight, 25,
	...
	DoMethod(CM_rows, MUIM_Notify, MUIA_Selected, TRUE, CM_columns, 3, MUIM_Set, MUIA_Disabled, TRUE);
	DoMethod(CM_rows, MUIM_Notify, MUIA_Selected, FALSE, CM_columns, 3, MUIM_Set, MUIA_Disabled, FALSE);
	DoMethod(CM_columns, MUIM_Notify, MUIA_Selected, TRUE, CM_rows,  3, MUIM_Set, MUIA_Disabled, TRUE);
	DoMethod(CM_columns, MUIM_Notify, MUIA_Selected, FALSE, CM_rows,  3, MUIM_Set, MUIA_Disabled, FALSE);
	DoMethod(CM_rows, MUIM_Notify, MUIA_Selected, TRUE, STR_rownumber, 3, MUIM_Set, MUIA_Disabled, FALSE);
        DoMethod(CM_rows, MUIM_Notify, MUIA_Selected, FALSE, STR_rownumber, 3, MUIM_Set, MUIA_Disabled, TRUE);
        DoMethod(CM_columns, MUIM_Notify, MUIA_Selected, TRUE, STR_rownumber,  3, MUIM_Set, MUIA_Disabled, FALSE);
        DoMethod(CM_columns, MUIM_Notify, MUIA_Selected, FALSE, STR_rownumber,  3, MUIM_Set, MUIA_Disabled, TRUE);
	DoMethod(CM_horizspacing, MUIM_Notify, MUIA_Selected, TRUE, STR_horizspace, 3, MUIM_Set, MUIA_Disabled, FALSE);
	DoMethod(CM_horizspacing, MUIM_Notify, MUIA_Selected, FALSE, STR_horizspace, 3, MUIM_Set, MUIA_Disabled, TRUE);
	DoMethod(CM_vertspacing, MUIM_Notify, MUIA_Selected, TRUE, STR_vertspace, 3, MUIM_Set, MUIA_Disabled, FALSE);
	DoMethod(CM_vertspacing, MUIM_Notify, MUIA_Selected, FALSE, STR_vertspace, 3, MUIM_Set, MUIA_Disabled, TRUE);
	/* Sens notifications */
	DoMethod(CM_sens, MUIM_Notify, MUIA_Selected, TRUE, CM_rows, 3, MUIM_Set, MUIA_Disabled, TRUE);
	DoMethod(CM_sens, MUIM_Notify, MUIA_Selected, FALSE, CM_rows, 3, MUIM_Set, MUIA_Disabled, FALSE);
	DoMethod(CM_sens, MUIM_Notify, MUIA_Selected, TRUE, CM_rows, 3, MUIM_Set, MUIA_Selected, FALSE);
	DoMethod(CM_sens, MUIM_Notify, MUIA_Selected, TRUE, CM_columns, 3, MUIM_Set, MUIA_Disabled, TRUE);
	DoMethod(CM_sens, MUIM_Notify, MUIA_Selected, FALSE, CM_columns, 3, MUIM_Set, MUIA_Disabled, FALSE);
	DoMethod(CM_sens, MUIM_Notify, MUIA_Selected, TRUE, CM_columns, 3, MUIM_Set, MUIA_Selected, FALSE);
	DoMethod(CM_sens, MUIM_Notify, MUIA_Selected, TRUE, CM_registermode, 3, MUIM_Set, MUIA_Disabled, TRUE);
	DoMethod(CM_sens, MUIM_Notify, MUIA_Selected, FALSE, CM_registermode, 3, MUIM_Set, MUIA_Disabled,group_aux->root);
	DoMethod(CM_sens, MUIM_Notify, MUIA_Selected, TRUE, CM_registermode, 3, MUIM_Set, MUIA_Selected, FALSE);
	/* columns & rows notifications */
	DoMethod(CM_columns, MUIM_Notify, MUIA_Selected, TRUE, CM_sens,  3, MUIM_Set, MUIA_Disabled, TRUE);
	DoMethod(CM_columns, MUIM_Notify, MUIA_Selected, TRUE, CM_sens,  3, MUIM_Set, MUIA_Selected, FALSE);
	DoMethod(CM_rows, MUIM_Notify, MUIA_Selected, TRUE, CM_sens,  3, MUIM_Set, MUIA_Disabled, TRUE);
	DoMethod(CM_rows, MUIM_Notify, MUIA_Selected, TRUE, CM_sens,  3, MUIM_Set, MUIA_Selected, FALSE);
	DoMethod(CM_columns, MUIM_Notify, MUIA_Selected, FALSE, CM_sens,  3, MUIM_Set, MUIA_Disabled, FALSE);
	DoMethod(CM_rows, MUIM_Notify, MUIA_Selected, FALSE, CM_sens,  3, MUIM_Set, MUIA_Disabled, FALSE);
	DoMethod(CM_columns, MUIM_Notify, MUIA_Selected, TRUE, CM_registermode,  3, MUIM_Set, MUIA_Disabled, TRUE);
        DoMethod(CM_columns, MUIM_Notify, MUIA_Selected, TRUE, CM_registermode,  3, MUIM_Set, MUIA_Selected, FALSE);
        DoMethod(CM_rows, MUIM_Notify, MUIA_Selected, TRUE, CM_registermode,  3, MUIM_Set, MUIA_Disabled, TRUE);
        DoMethod(CM_rows, MUIM_Notify, MUIA_Selected, TRUE, CM_registermode,  3, MUIM_Set, MUIA_Selected, FALSE);
        DoMethod(CM_columns, MUIM_Notify, MUIA_Selected, FALSE, CM_registermode,  3, MUIM_Set, MUIA_Disabled, group_aux->root);
        DoMethod(CM_rows, MUIM_Notify, MUIA_Selected, FALSE, CM_registermode,  3, MUIM_Set, MUIA_Disabled, group_aux->root);
	/* register notifications */
	DoMethod(CM_registermode, MUIM_Notify, MUIA_Selected, TRUE, gr_buttons, 3, MUIM_Set, MUIA_Disabled, FALSE );
	DoMethod(CM_registermode, MUIM_Notify, MUIA_Selected, FALSE, gr_buttons, 3, MUIM_Set, MUIA_Disabled, TRUE );
	DoMethod(CM_registermode, MUIM_Notify, MUIA_Selected, TRUE, lv_entries, 3, MUIM_Set, MUIA_Disabled, FALSE );
	DoMethod(CM_registermode, MUIM_Notify, MUIA_Selected, FALSE, lv_entries, 3, MUIM_Set, MUIA_Disabled, TRUE );
	DoMethod(CM_registermode, MUIM_Notify, MUIA_Selected, TRUE, CM_rows, 3, MUIM_Set, MUIA_Disabled, TRUE );
	DoMethod(CM_registermode, MUIM_Notify, MUIA_Selected, TRUE, CM_rows,  3, MUIM_Set, MUIA_Selected, FALSE);
	DoMethod(CM_registermode, MUIM_Notify, MUIA_Selected, FALSE, CM_rows,  3, MUIM_Set, MUIA_Disabled, FALSE);
	DoMethod(CM_registermode, MUIM_Notify, MUIA_Selected, TRUE, CM_columns, 3, MUIM_Set, MUIA_Disabled, TRUE );
	DoMethod(CM_registermode, MUIM_Notify, MUIA_Selected, TRUE, CM_columns,  3, MUIM_Set, MUIA_Selected, FALSE);
	DoMethod(CM_registermode, MUIM_Notify, MUIA_Selected, FALSE, CM_columns,  3, MUIM_Set, MUIA_Disabled, FALSE);
	DoMethod(CM_registermode, MUIM_Notify, MUIA_Selected, TRUE, CM_sens, 3, MUIM_Set, MUIA_Disabled, TRUE );
	DoMethod(CM_registermode, MUIM_Notify, MUIA_Selected, TRUE, CM_sens,  3, MUIM_Set, MUIA_Selected, FALSE);
	DoMethod(CM_registermode, MUIM_Notify, MUIA_Selected, FALSE, CM_sens,  3, MUIM_Set, MUIA_Disabled, FALSE);
	/* buttons notifications */
	DoMethod( bt_remove, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_DELETE);
	DoMethod( bt_down, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_DOWN);
	DoMethod( bt_up, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_UP);
	DoMethod( bt_new, MUIM_Notify, MUIA_Pressed, FALSE, STR_entry, 3, MUIM_Set, MUIA_Disabled, FALSE);
	DoMethod( bt_new, MUIM_Notify, MUIA_Pressed, FALSE,app, 2, MUIM_Application_ReturnID, ID_DBCLICK2);
	DoMethod( bt_new, MUIM_Notify, MUIA_Pressed, FALSE, WI_group, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_entry);
	DoMethod( bt_new, MUIM_Notify, MUIA_Pressed, FALSE, STR_entry, 3, MUIM_Set, MUIA_String_Contents, "");
	DoMethod(STR_entry, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_NEWOBJ);
	DoMethod(STR_entry, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, STR_entry, 3, MUIM_Set, MUIA_Disabled, TRUE);
	...
	DoMethod( WI_group, MUIM_Window_SetCycleChain, RegGroup1,
		  ...
		  CM_virtual, CM_columns, CM_rows, CM_horizspacing, CM_vertspacing, RegGroup2, STR_rownumber,
		  STR_horizspace, STR_vertspace, STR_label, lv_entries, bt_new, bt_remove, bt_up,
		  bt_down, STR_entry,
	...
	if (!new)
	{
                for (i=0;i<group_aux->entries->nb_elements;i++)
                {
                        ...
                        DoMethod( lv_entries, MUIM_List_Insert, &label_aux, 1, MUIV_List_Insert_Bottom);
		}
	}
        ...
        while (running)
	{
		...
		case ID_OKWIN:
			...
			else ErrorMessage(GetMUIBuilderString(MSG_NeedLabel));
			break;
		 case ID_DBCLICK:
                        get( lv_entries, MUIA_List_Active, &active);
                        set( STR_entry, MUIA_String_Contents, nth( group_aux->entries, active) );
                        dbclick = TRUE;
                        break;
		case ID_DBCLICK2:
                        dbclick = FALSE;
		...
		case ID_NEWOBJ:
                                ...
                                else
				{
                                ...
                                DoMethod( lv_entries, MUIM_List_Redraw, MUIV_List_Redraw_Active);
                                dbclick = FALSE;
                                }
                        }
                        break;
		case ID_DOWN:
                        ...
                        {
                                DoMethod( lv_entries, MUIM_List_Exchange, active, active+1);
                                down( group_aux->entries, active);
                                if (active < group_aux->entries->nb_elements) set( lv_entries, MUIA_List_Active, active+1);
                        }
                        else ErrorMessage(GetMUIBuilderString(MSG_SelectEntry));
                        break;
		case ID_UP:
                        get( lv_entries, MUIA_List_Active, &active);
                        if (active != MUIV_List_Active_Off)
                        {
                                DoMethod( lv_entries, MUIM_List_Exchange, active-1, active);
                                up( group_aux->entries, active);
                                if (active > 0) set( lv_entries, MUIA_List_Active, active-1);
                        }
                        else ErrorMessage(GetMUIBuilderString(MSG_SelectEntry));
                        break;
		case ID_DELETE:
                        ...
                        if (active != MUIV_List_Active_Off)
                        {
                                ...
                                DoMethod( lv_entries, MUIM_List_Remove, MUIV_List_Remove_Active);
                        }
                        else ErrorMessage(GetMUIBuilderString(MSG_SelectEntry));
                        break;
		case ID_END:
