...
BOOL NewCheckMark( check *check_aux, BOOL new )
{
	...
	if (!(GR_GroupArea = CreateGR_AreaGroup(&check_aux->Area, FALSE, TRUE, FALSE, TRUE, FALSE)))
		return(FALSE);
			...
			Child, RegGroup = RegisterGroup(RegisterTitles),
				Child, VGroup,
					Child, ColGroup(4),
						...
						Child, HVSpace,
					End,
					Child, ColGroup(2), GroupFrameT(GetMUIBuilderString(MSG_CheckMarkIdent)),
	...
	WI_current = WI_check;
        ...
        DoMethod(WI_check, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	...
	DoMethod(STR_title, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_check, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_label);
	DoMethod(STR_label, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_check, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_title);

	DoMethod( WI_check, MUIM_Window_SetCycleChain, RegGroup, CM_title, CM_state, STR_title, STR_label,
