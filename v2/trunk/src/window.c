...
void TestQuick( window * win )
{
	if (MakeTest)
	{
		...
		if (win->root.child->nb_elements>0)
		{
			...
			DoMethod( testwin, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, testwin, 3, MUIM_Set, MUIA_Window_Open, FALSE );
...
APTR LoadWindow( FILE *fichier, int version )
{
	...
	if (version>104) ReadNotify( fichier, win_aux );
	...
	if (version>104) win_aux->initopen = ReadInt(fichier);
	if (version>123) win_aux->nomenu   = ReadInt(fichier);
	if (version>125) win_aux->needmouse = ReadInt(fichier);
	if (version>=114)
	{
		i = ReadInt(fichier);
	...
	CopyObject( (object*) group_aux, (object*) &win_aux->root, sizeof(group), group_aux->father);
	if (strlen(group_aux->label)>0) win_aux->root.label[strlen(group_aux->label)] = '\0';
...
void CreateChain(object *obj,window *win)
{
	...
	case TY_GROUP:
	  group_aux = (group*) obj;
	  if (group_aux->generated) add(win->chain, obj);
	...
	case TY_SCALE:
	  break;
	default:
	  if( obj->generated) add(win->chain,obj);
	  break;
	}
}
...
void InitList( APTR list, APTR obj, int Depth, int MaxDepth)
{
	...
	case TY_GROUP:
		...
		if ((Depth<MaxDepth)||(group_aux->deplie))
		{
			...
			DoMethod( list, MUIM_List_Insert, &group_aux, 1, MUIV_List_Insert_Bottom );
			for(i=0;i<group_aux->child->nb_elements;i++)
			{
				InitList( list, nth( group_aux->child, i ), Depth+1, MaxDepth );
			}
		}
		else DoMethod( list, MUIM_List_Insert, &group_aux, 1, MUIV_List_Insert_Bottom );
		break;
	default:
...
{
	if (obj->id == TY_WINDOW)  return(FALSE);
	if (obj->father == parent) return(TRUE);
	else			   return(IsParent(obj->father,parent));
}
...
void EffaceObjet( APTR list, object *obj, int position )
{
	...
	case TY_GROUP:
		...
		if (group_aux->deplie) Toggle(obj, list, position+1);
...
void EditObject(APTR WI_window, APTR lv_group, window *win_aux)
{
	...
	DoMethod( lv_group, MUIM_List_GetEntry, MUIV_List_GetEntry_Active, &child_aux );
	if (child_aux)
	{
		if (config.phantom) set(WI_window,MUIA_Window_Open,FALSE);
		bool_aux = EditChild(child_aux);
		if (config.phantom) set(WI_window,MUIA_Window_Open,TRUE);
...
void ShowObject(APTR lv_group)
{
  ...
  if (MakeTest)
	...
	{
	  DoMethod(testwin, MUIM_Set, MUIA_Window_ActiveObject, child_aux->muiobj);
	}
    }
}
...
LONG DisplayTreeFunc(struct Hook *hook, char **array, object *obj)
{
	...
	concat[2] = '\0';
	buffer2[0] = '\0';
	if (obj->generated) strcpy(buffer3, "G");
	else		    strcpy(buffer3, " ");
	if (obj->Help.generated) strcpy(buffer4, "H");
	else			 strcpy(buffer4, " ");
	obj_aux = obj;
	...
	{
		group_aux = (group*) obj;
		if (!group_aux->deplie) strcpy ( buffer, "\033I[6:30]" );
		else		      	strcpy ( buffer, "\033I[6:39]" );
...
BOOL NewWindow( window *win_aux , BOOL new, object* obj2search )
{
	...
	WI_window = WindowObject,
			...
			MUIA_Window_Menu        , Menu,
				...
				Child, VGroup,
                                Child, HGroup,
					Child, ColGroup(2), GroupFrameT(GetMUIBuilderString(MSG_Objects)),
					Child, VGroup,
					...
					End,
					End,
					Child, VGroup, GroupFrameT(GetMUIBuilderString(MSG_Tmp)),
						MUIA_HorizWeight, 50,
				...
				Child, HGroup,
				Child, HSpace(0),
				Child, ColGroup(4), GroupFrameT( GetMUIBuilderString(MSG_Attributes) ),
	...
	DoMethod( bt_okwin, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	DoMethod( bt_notify, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_NOTIFY);
	DoMethod( bt_notify2, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_NOTIFY);
	DoMethod( bt_menu, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_NEW_SUBMENU);
	DoMethod( bt_cancelwin, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_END);
        DoMethod( bt_test, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_TEST);
	DoMethod( bt_rm, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_DELETE);
	...
	DoMethod( bt_totmp, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_TOTMP);
	DoMethod( bt_tochild, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_TOCHILD);
	DoMethod( bt_tmprm, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_TMPRM);
	DoMethod( bt_copy, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_COPY);
	DoMethod( bt_copytmp, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_COPYTMP);
	DoMethod( WI_window, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	DoMethod( STR_title, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_window, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_label);
	DoMethod( STR_label, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_window, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_title);
	DoMethod( lv_group, MUIM_Notify, MUIA_Listview_DoubleClick, TRUE, app, 2, MUIM_Application_ReturnID, ID_DBCLICK);
	DoMethod( bt_edit, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_EDIT);
	DoMethod( lv_group, MUIM_Notify, MUIA_List_Active, MUIV_EveryTime, app, 2, MUIM_Application_ReturnID, ID_UPDATE);
	DoMethod( CH_nomenu, MUIM_Notify, MUIA_Selected, MUIV_EveryTime, bt_menu, 3, MUIM_Set, MUIA_Disabled, MUIV_TriggerValue );

	DoMethod(WI_window, MUIM_Window_SetCycleChain, RegGroup, lv_group, lv_tmp, bt_notify, bt_add, bt_up,
		 bt_down, bt_rm,bt_edit, bt_copy, bt_copytmp, bt_totmp, bt_tochild,
		 bt_tmprm, bt_okwin, bt_test, bt_cancelwin, STR_label, STR_title, CH_appwindow,
		 CH_backdrop, CH_borderless, CH_closegadget, CH_depthgadget, CH_dragbar, CH_sizegadget,
		 CH_initopen, CH_nomenu, bt_notify2, bt_menu, NULL);
	...
	for (i=0;i<tmp->nb_elements;i++)
	{
		...
		DoMethod ( lv_tmp, MUIM_List_Insert, &chaine_aux, 1, MUIV_List_Insert_Bottom);
	}

	if (obj2search) i = 1000;
	else		i = config.depth;
	...
	else set(lv_group, MUIA_List_Active, MUIV_List_Active_Top);

	set(app,MUIA_Application_Sleep, TRUE);
        DoMethod(app,OM_ADDMEMBER, WI_window);
	if (!new) TestQuick( win_aux );
	...
	set(WI_window, MUIA_Window_ActiveObject, lv_group);
		...
		case ID_COPY:
			...
			if (child_aux)
			{
				...
				else ErrorMessage(GetMUIBuilderString(MSG_NotEnoughMemory));
			}
			else ErrorMessage(GetMUIBuilderString(MSG_SelectObject));
			break;
		case ID_COPYTMP:
			...
			{
				DoMethod(lv_group, MUIM_List_GetEntry, MUIV_List_GetEntry_Active, &obj_aux);
				if (obj_aux)
				{
					...
					if (obj_aux)
					{
						...
						AddToGroup(group_aux, lv_group, active, obj_aux);
			...
			else ErrorMessage(GetMUIBuilderString(MSG_SelectTmpObject));
			break;
		case ID_NOTIFY:
		...
		case ID_TOTMP:
			...
			if (active != MUIV_List_Active_Off)
			{
				...
				{
					ErrorMessage(GetMUIBuilderString(MSG_CantMovePopObjectChild));
					break;
				}
				if (child_aux->id == TY_GROUP)
				{
					...
					if (group_aux->root)
					{
						ErrorMessage(GetMUIBuilderString(MSG_CantMoveRoot));
						break;
					}
				}
				add(tmp, child_aux);
				...
				for(i=0;i<group_aux->child->nb_elements;i++)
				{
					if (child_aux == nth(group_aux->child, i)) break;
				}
		...
		case ID_UP:
			...
			if (obj_aux)
			{
				...
				for(i=0;i<group_aux->child->nb_elements;i++)
				{
					if (obj_aux == nth(group_aux->child, i)) break;
				}
				if (i == 0) break; /* on ne fait rien s'il est en tete de groupe */
				set(lv_group, MUIA_List_Quiet, TRUE);
				...
				{
					DoMethod(lv_group, MUIM_List_GetEntry, i, &child2_aux);
					if (child_aux == child2_aux) break;
				}
				AfficheObjet( lv_group, obj_aux, i);
				if (bool_aux) Toggle( obj_aux, lv_group, i+1);
				set(lv_group, MUIA_List_Active, i);
				set(lv_group, MUIA_List_Quiet, FALSE);
				WI_current = WI_window;
				TestQuick( win_aux );
		...
		case ID_DOWN:
			...
			if (obj_aux)
			{
				...
				if (i == group_aux->child->nb_elements-1) break;
				set(lv_group, MUIA_List_Quiet, TRUE);
				EffaceObjet(lv_group, obj_aux, active);
				...
				{
					DoMethod(lv_group, MUIM_List_GetEntry, i, &child2_aux);
					if (!IsParent(child2_aux, (group*)child_aux)) break;
				}
				AfficheObjet( lv_group, obj_aux, i);
				if (bool_aux) Toggle( obj_aux, lv_group, i+1);
				set(lv_group, MUIA_List_Active, i);
		...
		case ID_DELETE:
			...
			if ((obj_aux)&&bool_aux)
			{
				...
				{
					ErrorMessage(GetMUIBuilderString(MSG_CantDeletePopObjectChild));
					break;
				}
				if (obj_aux->id == TY_GROUP)
				{
					...
					if (group_aux->root)
					{
						ErrorMessage(GetMUIBuilderString(MSG_RemoveRootGroup));
						break;
					}
				}
				set(lv_group, MUIA_List_Quiet, TRUE);
			...
			else if (bool_aux) ErrorMessage(GetMUIBuilderString(MSG_SelectObject));
			break;
		case ID_TMPRM:
			get ( lv_tmp, MUIA_List_Active, &active);
			bool_aux = TRUE;
		...
		case ID_DBCLICK:
			...
			if (active == 2)
			{
				...
				else
				{
					...
					DoMethod(lv_group, MUIM_List_Redraw, MUIV_List_Redraw_Active);
					break;
				}
			}
			if (active==0)
		...
		case ID_EDIT:
			...
			else ErrorMessage(GetMUIBuilderString(MSG_QuitWindowAttributes));
                        break;
                case ID_TEST:
			aux = GetStr(STR_title);
                        strcpy (win_aux->title, aux);
			...
			else ErrorMessage(GetMUIBuilderString(MSG_NoChild));
                        break;
		case ID_INFO:
		...
		case ID_UNDO:
			...
			else ErrorMessage(GetMUIBuilderString(MSG_NothingToUndelete));
			break;
		case ID_NEW_SUBMENU:
			if (config.phantom) set(WI_window, MUIA_Window_Open, FALSE);
		...
		case ID_END:
			set( testwin, MUIA_Window_Open, FALSE );
			if (new) delete_nth(windows, windows->nb_elements-1 );
