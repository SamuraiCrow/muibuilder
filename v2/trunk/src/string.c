...
BOOL NewString( chaine *chaine_aux, BOOL new )
{
	...
	if (!(GR_GroupArea = CreateGR_AreaGroup(&chaine_aux->Area, FALSE, FALSE, TRUE, TRUE, TRUE)))
		return(FALSE);
		...
		MUIA_HelpNode, "Label",
			...
			Child, VGroup,
				...
				Child, CY_format = Cycle( STR_CY_format ),
	                        Child, ColGroup(2), GroupFrameT(GetMUIBuilderString(MSG_StringIdent)),
	...
	DoMethod( bt_ok, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
        ...
        DoMethod(WI_string, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	DoMethod(CM_title, MUIM_Notify, MUIA_Selected, TRUE, STR_title, 3, MUIM_Set, MUIA_Disabled, FALSE);
	DoMethod(CM_title, MUIM_Notify, MUIA_Selected, FALSE, STR_title, 3, MUIM_Set, MUIA_Disabled, TRUE);
	DoMethod(STR_title, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_string, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_label);
	DoMethod(STR_label, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_string, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_content);
	DoMethod(STR_content, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_string, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_accept);
	DoMethod(STR_accept, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_string, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_reject);
	DoMethod(STR_reject, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_string, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_maxlen);
	DoMethod(STR_maxlen, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_string, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_title);

	DoMethod(WI_string, MUIM_Window_SetCycleChain, RegGroup, CM_title,
		CM_secret, CM_integer, CY_format, STR_title, STR_label, STR_content, STR_accept,
		STR_reject, STR_maxlen, bt_ok, bt_cancel,
	...
	set(app,MUIA_Application_Sleep, TRUE);
        ...
        while (running)
        {
			...
			else ErrorMessage(GetMUIBuilderString(MSG_NeedLabelTitle));
                        break;
                case ID_END:
			running = FALSE;
                        break;
