...
APTR LoadListview( FILE* fichier, APTR father, int version )
{
	...
	if (version>104) ReadNotify( fichier, listview_aux );
	if (version<113)
	{
		listview_aux->Area.Weight = ReadInt(fichier);
	}
	if (version>=113) ReadArea(fichier, &listview_aux->Area);
...
BOOL NewListView( listview *list, BOOL new )
{
	...
	if (!(GR_GroupArea = CreateGR_AreaGroup(&list->Area, FALSE, TRUE, TRUE, TRUE, TRUE)))
		return(FALSE);
			...
			Child, RegGroup = RegisterGroup(RegisterTitles),
				...
				Child, VGroup,
				   Child, ColGroup(3),
					...
					End,
				    End,
				    Child, ColGroup(2), GroupFrameT(GetMUIBuilderString(MSG_ListIdent)),
						...
						Child, STR_title = StringObject,
							StringFrame,
							MUIA_String_Contents, list->title,
						End,
				    End,
				End,
	...
	WI_current = WI_list;
        ...
        DoMethod( WI_list, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_END);
	DoMethod( RA_type, MUIM_Notify, MUIA_Radio_Active, 1, STR_content, 3, MUIM_Set, MUIA_Disabled, FALSE);
	DoMethod( RA_type, MUIM_Notify, MUIA_Radio_Active, 0, STR_content, 3, MUIM_Set, MUIA_Disabled, TRUE);
	DoMethod( RA_type, MUIM_Notify, MUIA_Radio_Active, 2, STR_content, 3, MUIM_Set, MUIA_Disabled, TRUE);

	DoMethod(IM_constructhook, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_POPUP1 );
	DoMethod(IM_destructhook, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_POPUP2 );
	DoMethod(IM_comparehook, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_POPUP3 );
	DoMethod(IM_displayhook, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_POPUP4 );
	DoMethod(IM_multitesthook, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_POPUP5 );
	...
	DoMethod( WI_list, MUIM_Window_SetCycleChain, RegGroup,
		  ...
		  IM_constructhook, IM_destructhook, IM_comparehook, IM_displayhook, IM_multitesthook,
		  STR_format, STR_content, STR_label, STR_title,
	...
	DoMethod(app,OM_ADDMEMBER, WI_list);
        ...
        while (running)
        {
                ...
                {
		case ID_POPUP1:
			if (new) GetMUIarg( (object*) list, (APTR*) &list->constructhook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			else GetMUIarg( NULL, (APTR*) &list->constructhook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			if (list->constructhook) set(STR_constructhook, MUIA_Text_Contents, list->constructhook);
			else set(STR_constructhook, MUIA_Text_Contents, "");
			WI_current = WI_list;
			break;
		case ID_POPUP2:
			if (new) GetMUIarg( (object*) list, (APTR*) &list->destructhook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			else GetMUIarg( NULL, (APTR*) &list->destructhook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			if (list->destructhook) set(STR_destructhook, MUIA_Text_Contents, list->destructhook);
			else set(STR_destructhook, MUIA_Text_Contents, "");
			WI_current = WI_list;
			break;
		case ID_POPUP3:
			if (new) GetMUIarg( (object*) list, (APTR*) &list->comparehook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			else GetMUIarg( NULL, (APTR*) &list->comparehook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			if (list->comparehook) set(STR_comparehook, MUIA_Text_Contents, list->comparehook);
			else set(STR_comparehook, MUIA_Text_Contents, "");
			WI_current = WI_list;
			break;
		case ID_POPUP4:
			if (new) GetMUIarg( (object*) list, (APTR*) &list->displayhook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			else GetMUIarg( NULL, (APTR*) &list->displayhook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			if (list->displayhook) set(STR_displayhook, MUIA_Text_Contents, list->displayhook);
			else set(STR_displayhook, MUIA_Text_Contents, "");
			WI_current = WI_list;
			break;
		case ID_POPUP5:
			if (new) GetMUIarg( (object*) list, (APTR*) &list->multitesthook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			else GetMUIarg( NULL, (APTR*) &list->multitesthook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			if (list->multitesthook) set(STR_multitesthook, MUIA_Text_Contents, list->multitesthook);
			else set(STR_multitesthook, MUIA_Text_Contents, "");
			WI_current = WI_list;
			break;
                case ID_OKWIN:
