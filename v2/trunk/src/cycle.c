...
APTR LoadCycle(FILE* fichier, APTR father, int version )
{
	...
	if (version>104) ReadNotify( fichier, cycle_aux );
	if (version<113)
	{
		cycle_aux->Area.Weight = ReadInt(fichier);
	}
	if (version>=113) ReadArea( fichier, &cycle_aux->Area );
	do
	{
...
BOOL NewCycle ( cycle *cycle_aux, BOOL new)
{
	...
	DoMethod( bt_new, MUIM_Notify, MUIA_Pressed, FALSE, WI_cycle, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_entry);
	...
	DoMethod(STR_entry, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, STR_entry, 3, MUIM_Set, MUIA_Disabled, TRUE);
        DoMethod(WI_cycle, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	DoMethod( lv_entries, MUIM_Notify, MUIA_Listview_DoubleClick, TRUE, STR_entry, 3, MUIM_Set, MUIA_Disabled, FALSE);
	DoMethod( lv_entries, MUIM_Notify, MUIA_Listview_DoubleClick, TRUE, WI_cycle, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_entry);
        ...
        while (running)
	{
		...
		case ID_DBCLICK:
			...
			set( STR_entry, MUIA_String_Contents, nth( cycle_aux->entries, active) );
			dbclick = TRUE;
			break;
		case ID_DBCLICK2:
			dbclick = FALSE;
		...
		case ID_DOWN:
			...
			if (active != MUIV_List_Active_Off)
			{
				...
				if (active < cycle_aux->entries->nb_elements) set( lv_entries, MUIA_List_Active, active+1);
			}
		...
		case ID_DELETE:
			...
			break;
                case ID_OKWIN:
                        ...
                        if ((strlen(cycle_aux->label)>0)&&(cycle_aux->entries->nb_elements>0))
                        {
