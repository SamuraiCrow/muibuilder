...
void TestWindow( window* win )
{
	...
	for(i=0;i<windows->nb_elements;i++)
	{
		...
		if (win_aux!=win)
		{
			...
			DoMethod(win_aux->muiobj,
				...
				app,
				2,
				MUIM_Application_ReturnID, ID_CLOSE);
			DoMethod(win_aux->muiobj,
	...
	DoMethod( okwin, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_FINTEST);
	DoMethod( bt_ok, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_FINTEST);
	...
	set(okwin, MUIA_Window_Open , TRUE);
		...
		case ID_CLOSE:
			windows_opened--;
			if (windows_opened == 0) running=FALSE;
			break;
		case ID_OPEN:
			windows_opened++;
			break;
...
APTR CreateWindow( APTR objet )
{
	...
	case TY_WINDOW:
		...
		if (strlen( buffer )<4) strcat( buffer, "TEST" );
		result = CreateWindow(win->menu);
		if (win->nomenu)
		  result = WindowObject,
		  MUIA_Window_Title, win->title,
		  MUIA_Window_ID   , MAKE_ID(buffer[0],buffer[1],buffer[2],buffer[3]),
		  ...
		  MUIA_Window_RootObject, CreateWindow( &win->root ),
		  End;
		if (!result) ErrorMessage("Can't create Window");
		obj->muiobj = result;
		break;
	case TY_GROUP:
		grp = objet;
		if (strlen(grp->Area.TitleFrame)>0) chaine = grp->Area.TitleFrame;
		else chaine = NULL;
		if (grp->virtual)
		{
			if (!grp->registermode)
		...
		if (grp->virtual)
		{
			...
			if (grp->horizspacing) set( virt, MUIA_Group_HorizSpacing, grp->horizspace);
			if (grp->vertspacing) set( virt, MUIA_Group_VertSpacing, grp->vertspace);
			if ( grp->rows ) set( virt, MUIA_Group_Rows, grp->number );
			if ( grp->columns ) set( virt, MUIA_Group_Columns, grp->number);
		}
		else
		{
			set( result, MUIA_Group_Horiz, grp->horizontal);
			if (grp->horizspacing) set( result, MUIA_Group_HorizSpacing, grp->horizspace);
			if (grp->vertspacing) set( result, MUIA_Group_VertSpacing, grp->vertspace);
			if ( grp->rows ) set( result, MUIA_Group_Rows, grp->number );
			if ( grp->columns ) set( result, MUIA_Group_Columns, grp->number);
		}
		n = 0;
		if ((grp->rows)||(grp->columns))
		{
			n = grp->child->nb_elements%grp->number;
			if (n>0) n = grp->number - n;
			sprintf( buffer, GetMUIBuilderString(MSG_GroupError), grp->label, grp->child->nb_elements+n, grp->child->nb_elements);
			if ((n>0)&&(config.request)) ErrorMessage( buffer );
		}
		if ((grp->registermode)&&(grp->entries->nb_elements != grp->child->nb_elements))
		{
			sprintf( buffer, GetMUIBuilderString(MSG_GroupError), grp->label, grp->entries->nb_elements, grp->child->nb_elements);
			if (config.request) ErrorMessage( buffer );
			if (grp->entries->nb_elements>grp->child->nb_elements)
				n = grp->entries->nb_elements-grp->child->nb_elements;
		}
		chainon_aux = grp->child->head;
		for (i=0;i<grp->child->nb_elements;i++)
		{
			if (grp->virtual) DoMethod(virt, OM_ADDMEMBER, CreateWindow(chainon_aux->element));
			else DoMethod(result, OM_ADDMEMBER, CreateWindow(chainon_aux->element));
			chainon_aux = chainon_aux->next;
		}
		if (grp->child->nb_elements==0)
			if (grp->virtual) DoMethod(virt, OM_ADDMEMBER, HVSpace);
			else DoMethod(result, OM_ADDMEMBER, HVSpace);
		for (i=0;i<n;i++)
		{
			if (grp->virtual) DoMethod(virt, OM_ADDMEMBER, HVSpace);
			else DoMethod(result, OM_ADDMEMBER, HVSpace );
	...
	case TY_LISTVIEW:
		...
		if (listeview->multiselect) i = MUIV_Listview_MultiSelect_Default;
		else			    i = MUIV_Listview_MultiSelect_None;
		if (strlen(listeview->title)>0) chaine2 = listeview->title;
		else		      		chaine2 = NULL;
	...
	case TY_DIRLIST:
		dirlist_aux = objet;
		if (dirlist_aux->multi) i = MUIV_Listview_MultiSelect_Default;
		else			i = MUIV_Listview_MultiSelect_None;
		if (strlen(dirlist_aux->Area.TitleFrame)>0) chaine = dirlist_aux->Area.TitleFrame;
		else					    chaine = NULL;
		ParsePatternNoCase(dirlist_aux->accept, buffer, 256);
		if (strlen(dirlist_aux->accept)>0) chaine2 = buffer;
		else		      chaine2 = NULL;
		ParsePatternNoCase(dirlist_aux->reject, buffer2, 80);
		if (strlen(dirlist_aux->reject)>0) chaine3 = buffer2;
		else		       chaine3 = NULL;
		result = ListviewObject,
	...
	case TY_CHECK:
		...
		result = ColGroup(2),
			...
			Child, KeyLabel1(check_aux->title, check_aux->Area.key),
	...
	case TY_SLIDER:
		...
		result = GroupObject,
			...
			Child, KeyLabel(slider_aux->title, slider_aux->Area.key),
	...
	case TY_STRING:
		string_aux = objet;
		if (strlen(string_aux->Area.TitleFrame)>0) chaine = string_aux->Area.TitleFrame;
		else chaine = NULL;
		...
		result = ColGroup(2),
			...
			Child, aux = StringObject, StringFrame, 
				...
				End,
			End;
		if (strlen(string_aux->accept)>0) set(aux, MUIA_String_Accept, string_aux->accept);
		if (strlen(string_aux->reject)>0) set(aux, MUIA_String_Reject,  string_aux->reject);
		obj->muiobj = aux;
		}
		else
		{
		result = StringObject, StringFrame,
			 ...
			 MUIA_ControlChar, string_aux->Area.key,
                         End;
		if (strlen(string_aux->accept)>0) set(result, MUIA_String_Accept, string_aux->accept);
		if (strlen(string_aux->reject)>0) set(result, MUIA_String_Reject, string_aux->reject);
		obj->muiobj = result;
		}
		break;
	case TY_LABEL:
	...
	case TY_RADIO:
		radio_aux = objet;
		if (strlen(radio_aux->Area.TitleFrame)) chaine = radio_aux->Area.TitleFrame;
		else			      chaine = NULL;
		result = RadioObject,
	...
	case TY_RECTANGLE:
		...
		if (strlen(chaine) == 0) chaine = NULL;
		bool_aux = (rect_aux->type == 1)&&(strlen(rect_aux->Area.TitleFrame)>0);
		...
		if ((rect_aux->fixheight)&&(!rect_aux->fixwidth)&&(!bool_aux))
		{
			result = RectangleObject,
                                MUIA_Rectangle_VBar, (rect_aux->type == 2),
                                MUIA_Rectangle_HBar, (rect_aux->type == 1),
		...
		if (bool_aux) result = MUI_MakeObject(MUIO_BarTitle, rect_aux->Area.TitleFrame);
		obj->muiobj = result;
		break;
	case TY_COLORFIELD:
		field_aux = objet;
	...
	case TY_POPOBJECT:
		...
		result = PopobjectObject,
			MUIA_Popstring_String, String("",80),
			MUIA_Popstring_Button, PopButton( popobj_aux->image+MUII_PopUp ),
	...
	case TY_MENU:
		...
		{
			DoMethod(result, MUIM_Family_AddTail, CreateWindow(chainon_aux->element));
			chainon_aux = chainon_aux->next;
		}
	...
	case TY_SUBMENU:
		...
		result = MenuitemObject,
			...
			MUIA_Menu_Enabled, menu_aux->menu_enable,
		End;
		chainon_aux = menu_aux->childs->head;
		for(i=0;i<menu_aux->childs->nb_elements;i++)
		{
