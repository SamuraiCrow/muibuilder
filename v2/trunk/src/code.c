...
{
  if ((objdest == objsrc)||(objsrc->id == TY_APPLI)) return(TRUE);
  else
    if (objdest->id == TY_APPLI) return(FALSE);
    else return(IsParentFrom(objsrc, objdest->father));
}

LONG PopObjectDepth(popobject* popobj)
{
  if(popobj->obj->id == TY_POPOBJECT) return(PopObjectDepth((popobject*) popobj->obj)+1);
  else return(0);
}
  ...
  for(i=0;i<list->nb_elements;i++)
    {
      if (generate_all)
	{
	  if (!chainon_aux->ident) nb_identificateurs++;
	  chainon_aux->ident = 1;
	}
      else
      if ((chainon_aux->element==content))
	{
	  if (!chainon_aux->ident) nb_identificateurs++;
	  chainon_aux->ident = 1;
	  break;
...
void CalculeNbIdent(APTR obj)
{
  ...
  switch (obj_aux->id)
    {
    case TY_APPLI:
      if (application.appmenu->childs->nb_elements>0) CalculeNbIdent(application.appmenu);
    ...
    case TY_WINDOW:
      win_aux = obj;
      if (win_aux->menu->childs->nb_elements>0) CalculeNbIdent(win_aux->menu);
      CalculeNbIdent(&win_aux->root);
      break;
    case TY_GROUP:
      group_aux = obj;
      if (group_aux->registermode) nb_identificateurs++;
    ...
    case TY_LISTVIEW:
      list_aux = obj;
      if ((list_aux->type == 1)&&(strlen(list_aux->content)>0)) nb_identificateurs++;
      break;
    case TY_STRING:
      string_aux = obj;
      if (string_aux->title_exist) nb_identificateurs = nb_identificateurs + 2;
      break;
    case TY_SLIDER:
      slider_aux = obj;
      if (slider_aux->title_exist) nb_identificateurs = nb_identificateurs + 2;
      break;
    case TY_CHECK:
      check_aux = obj;
      if (check_aux->title_exist) nb_identificateurs = nb_identificateurs + 2;
      break;
    case TY_MENU:
    case TY_SUBMENU:
      menu_aux = obj;
...
void GenerateLabels(APTR obj, FILE *fichier)
{
  ...
  switch (obj_aux->id)
    {
    case TY_APPLI:
      if (application.appmenu->childs->nb_elements>0) GenerateLabels(application.appmenu, fichier);
      for(i=0;i<windows->nb_elements;i++)
	{
    ...
    case TY_WINDOW:
      win_aux = obj;
      if (win_aux->menu->childs->nb_elements>0) GenerateLabels(win_aux->menu, fichier);
      GenerateLabels(&win_aux->root, fichier);
      break;
    case TY_GROUP:
      group_aux = obj;
      if (group_aux->registermode) fprintf(fichier, "%cSTR_%s%c", TYPEVAR_TABSTRING, obj_aux->label, 0);
      for(i=0;i<group_aux->child->nb_elements;i++)
	{
      ...
      break;
    case TY_CYCLE:
      fprintf(fichier, "%c%sContent%c", TYPEVAR_TABSTRING,obj_aux->label,0);
      break;
    case TY_TEXT:
      fprintf(fichier, "%cSTR_%s%c", TYPEVAR_STRING,obj_aux->label,0);
      break;
    case TY_RADIO:
      fprintf(fichier,"%c%sContent%c",TYPEVAR_TABSTRING, obj_aux->label,0);
      break;
	...
	fprintf(fichier, "%c%sContent%c",TYPEVAR_STRING, obj_aux->label,0);
      break;
    case TY_POPOBJECT:
      popobj_aux = obj;
      fprintf(fichier, "%cSTR_%s%c", TYPEVAR_PTR, obj_aux->label, 0);
	...
	{
	  fprintf(fichier, "%cobj_aux%d%c", TYPEVAR_LOCAL_PTR,nb_var_aux++,0);
	  fprintf(fichier, "%cobj_aux%d%c", TYPEVAR_LOCAL_PTR,nb_var_aux++,0);
	}
      break;
    case TY_CHECK:
...
void GenerateInitLabels(APTR obj, FILE *fichier)
{
  ...
  switch (obj_aux->id)
    ...
    case TY_WINDOW:
      win_aux = obj;
      if (win_aux->menu->childs->nb_elements>0) GenerateInitLabels(win_aux->menu, fichier);
      if(win_aux->generated) fprintf(fichier, "%c",0);
      ...
      if (group_aux->registermode)
	{
	  if (group_aux->entries->nb_elements != 0)
	    ...
	    {
	      if (!local) fprintf(fichier, "Error%c", 0);
	      else        fprintf(fichier, "%s%s0%c", catprepend, group_aux->label, 0);
	      fprintf(fichier, "%c", 0);
	    }
	}
      for(i=0;i<group_aux->child->nb_elements;i++)
    ...
    case TY_TEXT:
      text_aux = obj;
      fprintf(fichier, "%c",0);
      if ((!local)||(strlen(text_aux->content)==0)) fprintf(fichier, "%s%c", text_aux->content,0);
      else	    fprintf(fichier, "%s%s%c",catprepend, text_aux->label,0);
      break;
    case TY_RADIO:
      radio_aux = obj;
      fprintf(fichier, "%c",0);
...
void VerifyLabels(void)
{
  ...
  for(i=0;i<labels->nb_elements-1;i++)
      ...
      if (obj1->id==TY_GROUP)
	{
	  ...
	  if (group_aux->registermode &&
	      (group_aux->entries->nb_elements != group_aux->child->nb_elements))
	    {
	      sprintf(buffer, GetMUIBuilderString(MSG_GroupError), group_aux->label, group_aux->entries->nb_elements, group_aux->child->nb_elements);
	      ErrorMessage(buffer);
	    }
	}
      ...
      if (!bool_aux)
	{
	  ...
	  DoMethod(lv_multi, MUIM_List_Insert, &chaine_aux, 1, MUIV_List_Insert_Bottom);
	}
  ...
  if (multiple_label)
      ...
      while (running)
	{
	  switch (DoMethod(app,MUIM_Application_Input,&signal))
		...
		{
		  obj_aux = nth(multi, i);
		  if ((obj_aux->id==TY_MENU)||(obj_aux->id==TY_MENU)||(obj_aux->id==TY_SUBMENU))
		    {
		      NewMenu(ParentMenu((menu*)obj_aux), FALSE);
		    }
		  else NewWindow(ParentWindow(obj_aux), FALSE, obj_aux);
		}
	      break;
	    case ID_END:
	      running = FALSE;
...
void CreateLabels(APTR obj_aux, window *win)
{
  ...
  switch(obj->id)
    ...
    case TY_APPLI:
      add(labels, obj_aux);
      if (application.appmenu->childs->nb_elements>0) CreateLabels(application.appmenu, NULL);
      /* all windows EXCEPT win */
      for(i=0;i<windows->nb_elements;i++)
	{
	  win_aux = nth(windows, i);
	  if (win_aux!=win) CreateLabels(win_aux, NULL);
	}
      break;
    case TY_WINDOW:
      win_aux = obj_aux;
      add(labels, win_aux);
      if (win_aux->menu->childs->nb_elements>0) CreateLabels(win_aux->menu, NULL);
...
int NumLabel(APTR obj)
{
  ...
  for(nb= 0;(obj != obj1)&&chainon_aux->next;)
	...
	case TY_LISTVIEW:
	  ...
	  if ((listview_aux->type == 1)&&(strlen(listview_aux->content)>0)) nb++;
	case TY_CHECK:
	  check_aux = (check*) obj1;
	  if (check_aux->title_exist) nb = nb + 2;
	  break;
	case TY_SLIDER:
	  slider_aux = (slider*) obj1;
	  if (slider_aux->title_exist) nb = nb + 2;
	  break;
	case TY_STRING:
	  string_aux = (chaine*) obj1;
	  if (string_aux->title_exist) nb = nb + 2;
	  break;
	case TY_GROUP:
	  group_aux = (group *)obj1;
	  if (group_aux->registermode) nb++;
	  break;
	}
      nb++;
      chainon_aux = chainon_aux->next;
...
void WriteObjArg(FILE *fichier, object *obj)
{
  ...
  switch(obj->id)
    ...
    case TY_STRING:
      ...
      if (string_aux->title_exist) i++;
      break;
    case TY_SLIDER:
      slider_aux = (slider*) obj;
      if (slider_aux->title_exist) i++;
      break;
    case TY_CHECK:
      check_aux = (check*) obj;
      if (check_aux->title_exist) i++;
      break;
    }
	     ...
	     )
{
  ...
  if (Background)
    {
      i = Area->Background;
      if (i>=MUII_BACKGROUND) i = i - 120;
      if (i>MUII_SelectedBack) i = i + 48;
      else			 i = i + MB_MUII_WindowBack;
  ...
  if ((key)&&(Area->key!='\0'))
    {
      ...
      if (local)	WriteLocaleChar	(fichier, obj->label);
      else		WriteChar	(fichier, Area->key);
    }
  if ((TitleFrame)&&(strlen(Area->TitleFrame)>0))
    {
      WriteAttribut   (fichier, MB_MUIA_FrameTitle);
      if (local)	WriteLocaleString2(fichier, obj->label);
      else		WriteString	 (fichier, Area->TitleFrame);
    }
}
...
void CodeCreate(object *obj, FILE *fichier)
{
  ...
  switch (obj->id)
      ...
      if (strlen(application.copyright)>0)
	{
	  ...
	  if (local) WriteLocaleString(fichier, "AppCopyright");
	  else	   WriteString	    (fichier, application.copyright);
	}
      if (strlen(application.description)>0)
	{
	  WriteAttribut	(fichier, MB_MUIA_Application_Description);
	  if (local) WriteLocaleString(fichier, "AppDescription");
	  else	   WriteString	    (fichier, application.description);
	}
    ...
    case TY_WINDOW:
      ...
      if (local) 	WriteLocaleString(fichier, win_aux->label);
      else		WriteString(fichier, win_aux->title);
      ...
      if (group_aux->virtual)
	{
	  ...
	  WriteEnd (fichier, MB_End);
	  if (group_aux->registermode) WriteEnd (fichier, MB_End);
	}
      break;
    case TY_STRING:
      string_aux = (chaine *) obj;
      ...
      if (string_aux->format)
	{
	  ...
	  WriteMUIArgAttribute(fichier, string_aux->format + MB_MUIV_String_Format_Left);
	}
      ...
      if (string_aux->title_exist)
	{
	  ...
	  if (local)	WriteLocaleString(fichier, string_aux->label);
	  else		WriteString	 (fichier, string_aux->title);
	  if (string_aux->Area.key!='\0')
	    if (local)	WriteLocaleChar (fichier, string_aux->label);
	    else		WriteChar	(fichier, string_aux->Area.key);
	  WriteEndFunction(fichier);
    ...
    case TY_LABEL:
	  ...
	  (label_aux->Area.Hide)	 ||
	  (label_aux->Area.Disable)	 ||
	  (label_aux->Area.Phantom))
	{
	  ...
	  if (local)	WriteLocaleString(fichier, label_aux->label);
	  else		WriteString	 (fichier, label_aux->title);
      ...
      else
	{
	  ...
	  if (local)	WriteLocaleString(fichier, label_aux->label);
	  else		WriteString	 (fichier, locale_string);
	  WriteEndFunction(fichier);
	}
      break;
    case TY_SPACE:
    ...
    case TY_KEYBUTTON:
	  ...
	  (strchr(button_aux->title, tolower(button_aux->Area.key))||
	   strchr(button_aux->title, toupper(button_aux->Area.key))))
	{
	  WriteObjFunction(fichier, MB_SimpleButton);
	  CreateLocaleString(button_aux->title, button_aux->Area.key);
	  if (local)	WriteLocaleString(fichier, button_aux->label);
	  else		WriteString	 (fichier, locale_string);
	  WriteEndFunction(fichier);
	}
      else
	{
	  ...
	  WriteAttribut	(fichier, MB_MUIA_Text_Contents);
	  if (local)	WriteLocaleString(fichier, button_aux->label);
	  else		WriteString	 (fichier, button_aux->title);
	  WriteAttribut	(fichier, MB_MUIA_Text_PreParse);
	  WriteString	(fichier, "\\033c");
	  if (button_aux->Area.key != '\0')
	    {
	      WriteAttribut(fichier, MB_MUIA_Text_HiChar);
	      if (!local) WriteChar (fichier, button_aux->Area.key);
	      else	  WriteLocaleChar(fichier, button_aux->label);
	    }
	  if (obj->Help.generated)
	    {
	      WriteAttribut(fichier, MB_MUIA_HelpNode);
      ...
      if (strlen(image_aux->spec)>0)
	{
      ...
      else WriteInteger(fichier, image_aux->type);
      if (obj->Help.generated)
	{
	  WriteAttribut(fichier, MB_MUIA_HelpNode);
	  WriteString(fichier, obj->label);
      ...
      if (strlen(listview_aux->title)>0)
	{
	  ...
	  if (local)	WriteLocaleString(fichier, listview_aux->label);
	  else		WriteString(fichier, listview_aux->title);
	}
      ...
      if (listview_aux->select)
	{
	  ...
	  WriteMUIArgAttribute(fichier, MB_MUIV_List_Active_Off + listview_aux->select);
	}
      ...
      if (listview_aux->comparehook)
	{
	  ...
	  UpdateNodeIdent (application.Functions, listview_aux->comparehook);
	}
      if (listview_aux->constructhook)
	{
	  WriteAttribut	(fichier, MB_MUIA_List_ConstructHook);
	  WriteExternalFunction(fichier, listview_aux->constructhook);
	  UpdateNodeIdent (application.Functions, listview_aux->constructhook);
	}
      if (listview_aux->destructhook)
	{
	  ...
	  UpdateNodeIdent (application.Functions, listview_aux->destructhook);
	}
      if (listview_aux->multitesthook)
	{
	  WriteAttribut	(fichier, MB_MUIA_List_MultiTestHook);
	  WriteExternalFunction(fichier, listview_aux->multitesthook);
	  UpdateNodeIdent (application.Functions, listview_aux->multitesthook);
	}
      ...
      if (listview_aux->multiselect)
	{
	  ...
	  WriteMUIArgAttribute	(fichier, MB_MUIV_Listview_MultiSelect_Default);
	}
	...
	{
	  if (check_aux->Area.key != '\0') WriteObjFunction(fichier, MB_KeyCheckMark);
	  else				 WriteObjFunction(fichier, MB_CheckMark);
	  WriteBool	(fichier, check_aux->init_state );
	  if (check_aux->Area.key != '\0')
	    {
	      if (local) WriteLocaleChar(fichier, check_aux->label);
	      else	   WriteChar	  (fichier, check_aux->Area.key);
	    }
	  WriteEndFunction(fichier);
	}
      else
      ...
      if (check_aux->title_exist)
	{
	  ...
	  if (local)      WriteLocaleString(fichier, check_aux->label);
	  else            WriteString      (fichier, check_aux->title);
	  if (check_aux->Area.key!='\0')
	    if (local)	WriteLocaleChar (fichier, check_aux->label);
	    else		WriteChar	(fichier, check_aux->Area.key);
	  WriteEndFunction(fichier);
      ...
      if (dirlist_aux->filterhook)
	{
	  ...
	  UpdateNodeIdent         (application.Functions, dirlist_aux->filterhook);
	}
      ...
      if (dirlist_aux->sortdirs)
	{
	  ...
	  WriteMUIArgAttribute	(fichier, MB_MUIV_Dirlist_SortDirs_First + dirlist_aux->sortdirs);
	}
      ...
      if (dirlist_aux->sorttype)
	{
	  ...
	  WriteMUIArgAttribute	(fichier, MB_MUIV_Dirlist_SortType_Name + dirlist_aux->sorttype);
	}
      WriteEnd		(fichier, MB_End);
      WriteVarAffect(fichier, NumLabel(dirlist_aux));
      WriteCreateObj	(fichier, MB_ListviewObject);
      ...
      if (strlen(gauge_aux->infotext)>0)
	{
	  ...
	  if (local) WriteLocaleString(fichier, gauge_aux->label);
	  else       WriteString      (fichier, gauge_aux->infotext);
	}
    ...
    case TY_RECTANGLE:
      ...
      if ((rect_aux->type != 1)||(strlen(rect_aux->Area.TitleFrame)==0))
	{
      ...
      else
	{
	  ...
	  else	WriteString	  (fichier, rect_aux->Area.TitleFrame);
	  WriteEndFunction(fichier);
	}
      break;
    case TY_COLORFIELD:
      ...
      else
	{
	  ...
	  WriteString     (fichier, "");
	  WriteInteger    (fichier, 80);
	  if (local) WriteLocaleChar(fichier, popasl->label);
	  else	 WriteChar(fichier, popasl->Area.key);
	  WriteEndFunction(fichier);
	}
      ...
      else
	{
	  ...
	  WriteString     (fichier, "");
	  WriteInteger    (fichier, 80);
	  if (local) WriteLocaleChar(fichier, popobj_aux->label);
	  else	 WriteChar(fichier, popobj_aux->Area.key);
	  WriteEndFunction(fichier);
	}
      ...
      if (popobj_aux->openhook)
	{
	  ...
	  UpdateNodeIdent (application.Functions, popobj_aux->openhook);
	}
      if (popobj_aux->closehook)
	{
	  ...
	  UpdateNodeIdent (application.Functions, popobj_aux->closehook);
	}
      WriteAttribut	(fichier, MB_MUIA_Popstring_String);
      ...
      if (strlen(menu_aux->name)>0)
	{
	  ...
	  if (local) WriteLocaleString(fichier, menu_aux->label);
	  else	   WriteString	    (fichier, menu_aux->name);
	}
	  ...
	  if (menu_aux->key)
	    {
	      ...
	      nospace[1] = '\0';
	      if (local) WriteLocaleCharAsString(fichier, menu_aux->label);
	      else	 WriteString	 (fichier, nospace);
	    }
...
{
  if (!PositionInParametersList(obj)) add(parameters, obj);
}
...
void WriteNotify(FILE * fichier, APTR objpar, APTR father)
{
  ...
  for(i=0;i<obj->notify->nb_elements;i++)
      ...
      if (!IsParentFrom(father, obj_aux))
	{
	  ...
	  num_label = nb_identificateurs+PositionInParametersList(obj_aux)-1;
	}
      else num_label = NumLabel(obj_aux);
      id_dest = obj_aux->id;
	...
	{
	case ARG_BOOL:
	  WriteBool(fichier, CEvenements[id_src][2*evt->src_type+1]);
	  break;
	case ARG_INT:
	  WriteInteger(fichier, CEvenements[id_src][2*evt->src_type+1]);
	  break;
	case ARG_MUI:
	  WriteMUIArgAttribute(fichier, CEvenements[id_src][2*evt->src_type+1]);
	  break;
	}
      switch (TYActions[id_dest][evt->dest_type])
	{
	case TY_NOTHING:
	  if (id_dest == 0) WriteVarArg (fichier, 0);
	  else		    WriteVarArg (fichier, num_label);
	  ...
	  if (CActions[id_dest][4*evt->dest_type+0]>2)
	      ...
	      if ((aux != 0)&&(aux != 1))
		{
	      ...
	      else WriteBool(fichier, aux);
	    }
	  else if (CActions[id_dest][4*evt->dest_type+0]>1) WriteMUIArgAttribute(fichier, CActions[id_dest][4*evt->dest_type+2]);
	  WriteEndNotification(fichier);
	  break;
	case TY_WINOBJ:
	  obj_aux2 = evt->arguments;
	  if (!bool_aux) WriteVarArg(fichier,
				     nb_identificateurs+
				     PositionInParametersList(obj_aux)-1
				    );
	  else          WriteVarArg (fichier, NumLabel(obj_aux2));
	  WriteInteger(fichier, CActions[id_dest][4*evt->dest_type+0]);
	  WriteMUIArg (fichier, CActions[id_dest][4*evt->dest_type+1]);
	...
	case TY_CONS_STRING:
	  ...
	  else WriteString (fichier, evt->argstring);
	  WriteEndNotification(fichier);
	  break;
	case TY_CONS_BOOL:
	  WriteVarArg (fichier, num_label);
...
void GenerateCodeObject(APTR obj_aux)
{
  ...
  if (Requester(title, dir, file))
	...
	{
	  fprintf(fichier, "%d%c", nb_identificateurs+parameters->nb_elements, 0);
	  ...
	  GenerateListLabels(fichier, application.Variables, TYPEVAR_EXTERNAL);
	  fclose(fichier);
	}
      if (fichier = fopen("T:MUIBuilder3.tmp", "w+"))
	{
      ...
      strcat (buffer, "\"");
      set(TextInfo, MUIA_Text_Contents, GetMUIBuilderString(MSG_GenerateSource));
      SystemTags(buffer,
...
void GenerateCode(queue *windows)
{
  ...
  if (fichier = fopen("T:MUIBuilder1.tmp" , "w+"))
      ...
      if (strlen(real_getstring)==0)
	{
      ...
      fprintf(fichier, "%s%c", real_getstring, 0);
      fprintf(fichier, "%s%c", catfile, 0);
      fprintf(fichier, "%d%d%d%d%d%d%c", env, declarations, code, local, notifications, 1, 0);
      fclose(fichier);
    }
  nb_identificateurs = 0;
  numfirstlabel = 0;
...
void Code()
{
  APTR	WI_code, lv_labels, GR_grp_0, bt_app, bt_object, bt_remove, lv_genlabels;
  APTR	bt_add, bt_quit, bt_catalog, RegGroup, STR_CatalogPrepend;
  APTR	CM_declarations, CM_code, CM_env, CM_local, CM_notifications, CM_genall;
  ...
  WI_code = WindowObject,
	...
	Child, GroupObject,
	  Child, GroupObject,
	    ...
	    Child, lv_labels  = ListviewObject, GroupFrameT(GetMUIBuilderString(MSG_ObjectsLabels)),
	      MUIA_Listview_List, ListObject,
		InputListFrame,
	      End,
	    End,
	    Child, lv_genlabels = ListviewObject, GroupFrameT(GetMUIBuilderString(MSG_GeneratedLabels)),
	      MUIA_Listview_List, ListObject,
		InputListFrame,
	      End,
	    End,
	...
	End,
	Child, VGroup,
	  Child, ColGroup(4), GroupFrameT(GetMUIBuilderString(MSG_Options)),
	    Child, HVSpace,
  ...
  DoMethod(bt_add,MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_NEWOBJ);
  DoMethod(bt_quit,MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_END);
  DoMethod(WI_code, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_END);
  DoMethod(bt_catalog,MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_CAT);
  DoMethod(STR_GetString, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_code, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_filename);
  ...
  for(i=0;i<labels->nb_elements;i++)
    {
      ...
      DoMethod(lv_labels, MUIM_List_Insert, &chaine_aux, 1, MUIV_List_Insert_Bottom);
      if (obj_aux->generated)
	{
	  add(genlabels, obj_aux);
	  DoMethod(lv_genlabels, MUIM_List_Insert, &chaine_aux, 1, MUIV_List_Insert_Bottom);
	}
    }
  ...
  while (running)
	...
	case ID_GENERATE:
	  if (windows->nb_elements>0)
	    {
	      ...
	      set(TextInfo, MUIA_Text_Contents, GetMUIBuilderString(MSG_CodeGeneration));
	  ...
	  else ErrorMessage(GetMUIBuilderString(MSG_NeedWindows));
	  break;
	case ID_GENERATEWIN:
	  ...
	  else ErrorMessage(GetMUIBuilderString(MSG_SelectLabel));
	  break;
	case ID_NEWOBJ:
	  ...
	  if (active != MUIV_List_Active_Off)
	      ...
	      while ((!bool_aux)&&(i<genlabels->nb_elements))
		{
	      ...
	      if (bool_aux) ErrorMessage(GetMUIBuilderString(MSG_AlreadyGenerated));
	      else
		{
		  add(genlabels, obj_aux);
		  chaine_aux = obj_aux->label;
	  ...
	  else ErrorMessage(GetMUIBuilderString(MSG_SelectObject));
	  break;
	case ID_DELETE:
	  get(lv_genlabels, MUIA_List_Active, &active);
	  bool_aux = TRUE;
	  if (active != MUIV_List_Active_Off)
	    {
	      if (config.request) bool_aux = RequestMessage(GetMUIBuilderString(MSG_Confirm));
	      if (bool_aux)
		{
