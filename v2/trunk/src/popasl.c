...
APTR LoadPopAsl( FILE* fichier, APTR father, int version)
{
	...
	if (version>=113) ReadArea( fichier, &popasl->Area);
...
BOOL NewPopAsl( popaslobj *popasl, BOOL new )
{
	...
	if (!(GR_GroupArea = CreateGR_AreaGroup(&popasl->Area, FALSE, TRUE, TRUE, TRUE, FALSE)))
		return(FALSE);
	...
	DoMethod( BT_ok, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
        ...
        DoMethod( WI_PopAsl, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	DoMethod( STR_label, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_PopAsl, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_starthook);
	DoMethod( STR_starthook, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_PopAsl, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_stophook);
	DoMethod( STR_stophook, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_PopAsl, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_label);
	DoMethod( IM_starthook, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_POPUP1 );
	DoMethod( IM_stophook, MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, ID_POPUP2 );

	DoMethod( WI_PopAsl, MUIM_Window_SetCycleChain, RegGroup, LV_images, RA_type,
		  IM_starthook, IM_stophook, STR_label,
	...
	{
		aux = STR_images[i];
		DoMethod( LV_images, MUIM_List_Insert, &aux, 1, MUIV_List_Insert_Bottom );
	}
	set( LV_images, MUIA_List_Active, popasl->image );
		...
		{
		case ID_POPUP1:
			if (new) GetMUIarg( (object*) popasl, (APTR*) &popasl->starthook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			else GetMUIarg( NULL, (APTR*) &popasl->starthook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			if (popasl->starthook) set(STR_starthook, MUIA_Text_Contents, popasl->starthook);
			else set(STR_starthook, MUIA_Text_Contents, "");
			WI_current = WI_PopAsl;
			break;
		case ID_POPUP2:
			if (new) GetMUIarg( (object*) popasl, (APTR*) &popasl->stophook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			else GetMUIarg( NULL, (APTR*) &popasl->stophook, NULL, application.Functions, TRUE, GetMUIBuilderString(MSG_ChoseFunction));
			if (popasl->stophook) set(STR_stophook, MUIA_Text_Contents, popasl->stophook);
			else set(STR_stophook, MUIA_Text_Contents, "");
			WI_current = WI_PopAsl;
			break;
		case ID_OKWIN:
