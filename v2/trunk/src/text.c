...
APTR LoadText ( FILE* fichier, APTR father, int version )
{
	...
	if (version>104) ReadNotify( fichier, text_aux );
	if (version<113)
	{
...
BOOL NewText( texte *text_aux, BOOL new)
{
	...
	if (!(GR_GroupArea = CreateGR_AreaGroup(&text_aux->Area, TRUE, TRUE, TRUE, TRUE, TRUE)))
		return(FALSE);
				...
				Child, GroupObject,
					...
					Child, ColGroup(2),
						...
						Child, STR_content = StringObject,
							...
							MUIA_String_Contents, text_aux->content,
						End,
						Child, Label2(GetMUIBuilderString(MSG_Label)),
						Child, STR_label = StringObject, StringFrame,
	...
	DoMethod(WI_text, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	...
	DoMethod( WI_text, MUIM_Window_SetCycleChain, RegGroup,
		 ...
		 GR_GroupArea->CH_Phantom, GR_GroupArea->SL_Weight, GR_GroupArea->CY_Background,
		 GR_GroupArea->CY_Frame, GR_GroupArea->STR_Char, GR_GroupArea->STR_TitleFrame, NULL);
