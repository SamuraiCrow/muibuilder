...
void NotifyCatalog( APTR obj, FILE *fichier )
{
  ...
  for(i=0;i<obj_aux->notify->nb_elements;i++)
	...
	{
	  fprintf(fichier, ";MUIB Notification string\n%s%sNotify%d(//)\n%s\n",
		  catprepend,
		  obj_aux->label,
		  i,
		  evt->argstring);
...
void ObjectCatalog( APTR obj, FILE *fichier )
{
  ...
  switch (obj_aux->id)
	...
	{
	  fprintf( fichier, ";MUIB group\n%s%sTitle (//)\n%s\n", catprepend, group_aux->label,group_aux->Area.TitleFrame);
	}
	  ...
	  strchr(button_aux->title, toupper(button_aux->Area.key))))
	{
	  ...
	  fprintf( fichier, "%s%s (//)\n%s\n", catprepend, button_aux->label, locale_string);
	}
      else
	{
	  if (button_aux->Area.key!='\0')
	    {
	      fprintf( fichier, "%s%s (//)\n%c\\00%s\n", catprepend, button_aux->label, button_aux->Area.key, button_aux->title);
	    }
	  else fprintf( fichier, "%s%s (//)\n%s\n", catprepend, button_aux->label,button_aux->title);
	}
      break;
    case TY_TEXT:
      text_aux = obj;
	...
	{
	  fprintf( fichier, ";MUIB text content\n%s%sTitle (//)\n%s\n", catprepend, text_aux->label, text_aux->Area.TitleFrame);
	}
      if ((strlen(text_aux->content)>0)||(text_aux->Area.key!='\0'))
	{
	  fprintf( fichier, ";MUIB text content\n%s%s (//)\n", catprepend, text_aux->label );
	  if (text_aux->Area.key!='\0') fprintf( fichier, "%c\\00", text_aux->Area.key );
	  if (strlen(text_aux->content)>0) fprintf( fichier, "%s", text_aux->content);
	  fprintf(fichier, "\n");
	}
      break;
    case TY_GAUGE:
	...
	{
	  fprintf( fichier, ";MUIB Radio TitleFrame\n%s%sTitle (//)\n%s\n", catprepend, radio_aux->label, radio_aux->Area.TitleFrame);
	}
      ...
      if ((check_aux->title_exist)||(check_aux->Area.key!='\0'))
	{
	  ...
	  fprintf( fichier, "%s%s (//)\n", catprepend, check_aux->label );
	  if (check_aux->Area.key!='\0') fprintf(fichier, "%c\\00", check_aux->Area.key);
	  if (check_aux->title_exist) fprintf( fichier, "%s", check_aux->title );
	  fprintf(fichier, "\n");
	}
      break;
    case TY_SLIDER:
	...
	{
	  fprintf( fichier, ";MUIB Slider TitleFrame\n%s%sTitle (//)\n%s\n", catprepend, slider_aux->label, slider_aux->Area.TitleFrame);
	}
      if ((slider_aux->title_exist)||(slider_aux->Area.key!='\0'))
	{
	  fprintf( fichier, ";MUIB Slider\n");
	  fprintf( fichier, "%s%s (//)\n", catprepend, slider_aux->label );
	  if (slider_aux->Area.key!='\0') fprintf(fichier, "%c\\00", slider_aux->Area.key);
	  if (slider_aux->title_exist) fprintf(fichier, "%s",slider_aux->title);
	  fprintf(fichier, "\n");
	}
      break;
    case TY_STRING:
	...
	{
	  fprintf( fichier, ";MUIB Slider TitleFrame\n%s%sTitle (//)\n%s\n", catprepend, string_aux->label, string_aux->Area.TitleFrame);
	}
      if ((string_aux->title_exist)||(string_aux->Area.key!='\0'))
	{
	  fprintf( fichier, ";MUIB String Gadget\n");
	  fprintf( fichier, "%s%s (//)\n", catprepend, string_aux->label);
	  if (string_aux->Area.key!='\0') fprintf(fichier, "%c\\00", string_aux->Area.key);
	  if (string_aux->title_exist) fprintf( fichier, "%s", string_aux->title);
	  fprintf(fichier, "\n");
	}
      break;
    case TY_LABEL:
      ...
      else
	{
	  if (label_aux->Area.key!='\0')
	    {
	      fprintf( fichier, ";MUIB Label\n%s%s (//)\n%c\\00%s\n", catprepend, label_aux->label, label_aux->Area.key, label_aux->title);
	    }
	  else fprintf( fichier, ";MUIB Label\n%s%s (//)\n%s\n", catprepend, label_aux->label, label_aux->title);
	}
      break;
    case TY_PROP:
      prop_aux = obj;
	...
	{
	  fprintf( fichier, ";MUIB Prop TitleFrame\n%s%sTitle (//)\n%s\n", catprepend, prop_aux->label, prop_aux->Area.TitleFrame);
	}
	...
	{
	  fprintf( fichier, ";MUIB Rectangle TitleFrame\n%s%sTitle (//)\n%s\n", catprepend, rect_aux->label, rect_aux->Area.TitleFrame);
	}
	...
	{
	  fprintf( fichier, ";MUIB ColorField TitleFrame\n%s%sTitle (//)\n%s\n", catprepend, field_aux->label, field_aux->Area.TitleFrame);
	}
	...
	{
	  fprintf( fichier, ";MUIB DirList TitleFrame\n%s%sTitle (//)\n%s\n", catprepend, dir_aux->label, dir_aux->Area.TitleFrame);
	}
	...
	{
	  fprintf( fichier, ";MUIB Listview TitleFrame\n%s%sTitle (//)\n%s\n", catprepend, Listview_aux->label, Listview_aux->Area.TitleFrame);
	}
      if ((strlen(Listview_aux->title)>0)||(Listview_aux->Area.key!='\0'))
	{
	  fprintf( fichier, ";MUIB Listview Gadget\n");
	  fprintf( fichier, "%s%s (//)\n", catprepend, Listview_aux->label);
	  if (Listview_aux->Area.key!='\0') fprintf(fichier, "%c\\00", Listview_aux->Area.key);
	  if (strlen(Listview_aux->title)>0) fprintf( fichier, "%s", Listview_aux->title);
	  fprintf(fichier, "\n");
	}
      if ((Listview_aux->type==1)&&(strlen(Listview_aux->content)>0))
	{
	  fprintf( fichier, ";MUIB Listview Gadget Initial Content\n");
	  fprintf( fichier, "%s%sContent (//)\n%s\n", catprepend, Listview_aux->label, Listview_aux->content );
	}
      break;
    case TY_POPASL:
      popasl_aux = obj;
    ...
    case TY_MENUITEM:
      menu_aux = obj;
      if (strncmp(menu_aux->name, "BarLabel", 8 ) == 0) break;
...
void CopyStrings(FILE *tmp, FILE *fichier)
{
  ...
  while(!feof(tmp))
	...
	{
	  if (strlen(string)>=5) ReadLine(tmp);
	  ReadLine(tmp);
	  ReadLine(tmp);
	}
      else
	{
	  ...
	  if (strlen(string)>=5) CopyLines(tmp, fichier, 1);
	}
    }
}
  ...
  if (fichier = fopen( catfile, "w+" ) )
    {
      ...
      fprintf( fichier, ";MUIB\n");
      fprintf( fichier, ";MUIB Please don't modify the lines generated by MUIBuilder,\n;MUIB but you can add your own entries.\n");
      fprintf( fichier, ";MUIB\n%sAppDescription (//)\n", catprepend);
      fprintf( fichier, "%s\n;MUIB\n",application.description );
      fprintf( fichier, "%sAppCopyright (//)\n%s\n", catprepend, application.copyright);
