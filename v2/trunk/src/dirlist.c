...
APTR LoadDirList( FILE* fichier, APTR father, int version)
{
	...
	if (version>104) ReadNotify( fichier, dirlist_aux );
	if (version<113) dirlist_aux->Area.Weight = ReadInt(fichier);
	else ReadArea(fichier, &dirlist_aux->Area);
...
BOOL NewDirList( dirlist *dirlist_aux, BOOL new )
{
	...
	if (!(GR_GroupArea = CreateGR_AreaGroup(&dirlist_aux->Area, FALSE, TRUE, TRUE, TRUE, TRUE)))
		return(FALSE);
	...
	WI_listdir = WindowObject,
		...
		WindowContents, VGroup,
			...
			Child, VGroup,
				Child, HGroup,
					Child,  GroupObject, GroupFrameT( GetMUIBuilderString(MSG_Attributes) ),
	...
	DoMethod(WI_listdir, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, app, 2, MUIM_Application_ReturnID, ID_OKWIN);
	DoMethod(STR_label, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_listdir, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_directory);
	DoMethod(STR_directory, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_listdir, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_accept);
	DoMethod(STR_accept, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_listdir, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_reject);
	DoMethod(STR_reject, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime, WI_listdir, 3, MUIM_Set, MUIA_Window_ActiveObject, STR_label);

	DoMethod( WI_listdir, MUIM_Window_SetCycleChain, RegGroup, CH_drawers,
		  CH_files, CH_filter, CH_multi, CH_icons, CH_highlow,
		  RA_sorttype, RA_sortdirs, STR_directory, STR_accept, STR_reject, STR_label,
	...
	while (running)
	{
		...
		case ID_OKWIN:
			...
			if (strlen(aux)>0)
			{
				...
				get( RA_sorttype, MUIA_Radio_Active, &dirlist_aux->sorttype);
				get( RA_sortdirs, MUIA_Radio_Active, &dirlist_aux->sortdirs);
